...
4. <span data-ttu-id="93ded-161">在 "**新建 SharePoint 相关应用程序**" 对话框中，填写以下三个字段：</span><span class="sxs-lookup"><span data-stu-id="93ded-161">In the **New app for SharePoint** dialog box, fill in the following three fields:</span></span> 
   ...
   - <span data-ttu-id="93ded-169">在 "**您希望如何承载 sharepoint 相关应用程序**" 下拉列表中，选择 " **sharepoint 托管**"。</span><span class="sxs-lookup"><span data-stu-id="93ded-169">In the **How do you want to host your app for SharePoint** drop-down list, choose **SharePoint-hosted**.</span></span>
    
   > [!CAUTION]
   > <span data-ttu-id="93ded-170">如果您在错误的情况下选择**提供程序托管**的默认项目类型，Visual Studio 将在解决方案中创建两个项目：一个**QuickStatus**项目和一个**QuickStatusWeb**项目。</span><span class="sxs-lookup"><span data-stu-id="93ded-170">If you choose the default **Provider-hosted** project type by mistake, Visual Studio creates two projects in the solution: a **QuickStatus** project and a **QuickStatusWeb** project.</span></span> <span data-ttu-id="93ded-171">如果看到两个项目，请删除该解决方案，然后重新开始。</span><span class="sxs-lookup"><span data-stu-id="93ded-171">If you see two projects, delete that solution and start again.</span></span> 
  
5. <span data-ttu-id="93ded-172">选择 **"确定"** 以创建**QuickStatus**解决方案、 **QuickStatus**项目和默认文件。</span><span class="sxs-lookup"><span data-stu-id="93ded-172">Choose **OK** to create the **QuickStatus** solution, **QuickStatus** project, and default files.</span></span> 
    
6. <span data-ttu-id="93ded-173">打开清单设计器视图（例如，双击 Appmanifest.xml 文件）。</span><span class="sxs-lookup"><span data-stu-id="93ded-173">Open the Manifest Designer view (for example, double-click the AppManifest.xml file).</span></span> <span data-ttu-id="93ded-174">在 "**常规**" 选项卡上，"**标题**" 文本框应显示您在步骤4中键入的应用名称。</span><span class="sxs-lookup"><span data-stu-id="93ded-174">On the **General** tab, the **Title** text box should show the app name that you typed in step 4.</span></span> <span data-ttu-id="93ded-175">选择 "**权限**" 选项卡，为应用程序添加以下权限请求（请参阅图2）：</span><span class="sxs-lookup"><span data-stu-id="93ded-175">Choose the **Permissions** tab to add the following permission requests for the app (see Figure 2):</span></span> 
    
   - <span data-ttu-id="93ded-176">在 "**权限请求**" 列表的第一行中，在 "**范围**" 列中，在下拉列表中选择 " **Statusing** "。</span><span class="sxs-lookup"><span data-stu-id="93ded-176">In the first row of the **Permission requests** list, in the **Scope** column, choose **Statusing** in the drop-down list.</span></span> <span data-ttu-id="93ded-177">在 "**权限**" 列中，选择 " **SubmitStatus**"。</span><span class="sxs-lookup"><span data-stu-id="93ded-177">In the **Permission** column, choose **SubmitStatus**.</span></span>
    
   - <span data-ttu-id="93ded-178">添加一个**范围**为**多个项目**且**权限**为 "已**读**" 的行。</span><span class="sxs-lookup"><span data-stu-id="93ded-178">Add a row where the **Scope** is **Multiple Projects** and the **Permission** is **Read**.</span></span>
    
   <span data-ttu-id="93ded-179">**图2。为 statusing 应用程序设置权限范围**</span><span class="sxs-lookup"><span data-stu-id="93ded-179">**Figure 2. Setting the permission scope for a statusing app**</span></span>

   <span data-ttu-id="93ded-180">![为状态应用程序设置权限范围](media/pj15_CreateStatusingApp_PermissionScope.gif "为状态应用程序设置权限范围")</span><span class="sxs-lookup"><span data-stu-id="93ded-180">![Setting the permission scope for a statusing app](media/pj15_CreateStatusingApp_PermissionScope.gif "Setting the permission scope for a statusing app")</span></span>
  
<span data-ttu-id="93ded-181">通过**QuickStatus**应用程序，Project Web app 用户可以从多个项目中读取该用户的分配，更改分配的完成百分比，并提交更新。</span><span class="sxs-lookup"><span data-stu-id="93ded-181">The **QuickStatus** app enables a Project Web App user to read assignments for that user from multiple projects, change the assignment percent complete, and submit the update.</span></span> <span data-ttu-id="93ded-182">此应用程序不需要图2中的下拉列表中显示的其他权限请求范围。</span><span class="sxs-lookup"><span data-stu-id="93ded-182">The other permission request scopes shown in the drop-down list in Figure 2 are not required for this app.</span></span> <span data-ttu-id="93ded-183">权限请求作用域是指代表用户请求的应用程序的权限。</span><span class="sxs-lookup"><span data-stu-id="93ded-183">The permission request scopes are the permissions that the app requests on behalf of the user.</span></span> <span data-ttu-id="93ded-184">如果用户在 Project Web App 中没有这些权限，则应用程序不会运行。</span><span class="sxs-lookup"><span data-stu-id="93ded-184">If the user does not have those permissions in Project Web App, the app does not run.</span></span> <span data-ttu-id="93ded-185">应用程序可以具有多个权限请求范围，包括用于其他 SharePoint 权限的范围，但只应具有应用程序功能所需的最低权限。</span><span class="sxs-lookup"><span data-stu-id="93ded-185">An app can have multiple permission request scopes, including those for other SharePoint permissions, but should have only the minimum necessary for the app functionality.</span></span> <span data-ttu-id="93ded-186">以下是与 Project Server 相关的权限请求范围：</span><span class="sxs-lookup"><span data-stu-id="93ded-186">Following are the permission request scopes that are related to Project Server:</span></span> 
...
### <a name="creating-the-html-content-for-the-quickstatus-app"></a><span data-ttu-id="93ded-196">为 QuickStatus 应用程序创建 HTML 内容</span><span class="sxs-lookup"><span data-stu-id="93ded-196">Creating the HTML content for the QuickStatus app</span></span>
...
- <span data-ttu-id="93ded-205">**Form**元素包含所有其他 UI 元素。</span><span class="sxs-lookup"><span data-stu-id="93ded-205">A **form** element contains all of the other UI elements.</span></span> 
...
1. <span data-ttu-id="93ded-220">在 Visual Studio 中，打开 "默认 .aspx" 文件。</span><span class="sxs-lookup"><span data-stu-id="93ded-220">In Visual Studio, open the Default.aspx file.</span></span>
   ...
   <span data-ttu-id="93ded-221">该文件包含两个**asp： Content**元素：具有该`ContentPlaceHolderID="PlaceHolderAdditionalPageHead"`属性的元素添加到页面页眉中，且具有该`ContentPlaceHolderID="PlaceHolderMain"`属性的元素放置在页面**正文**元素中。</span><span class="sxs-lookup"><span data-stu-id="93ded-221">The file includes two **asp:Content** elements: The element with the  `ContentPlaceHolderID="PlaceHolderAdditionalPageHead"` attribute is added within the page header, and the element with the  `ContentPlaceHolderID="PlaceHolderMain"` attribute is placed within the page **body** element.</span></span> 
...
- <span data-ttu-id="93ded-241">当文档对象模型（DOM）实例化时，将运行文档**就绪**事件处理程序。</span><span class="sxs-lookup"><span data-stu-id="93ded-241">The document **ready** event handler runs when the document object model (DOM) is instantiated.</span></span> <span data-ttu-id="93ded-242">**就绪**事件处理程序将执行以下四个步骤：</span><span class="sxs-lookup"><span data-stu-id="93ded-242">The **ready** event handler does the following four steps:</span></span> 
...
2. <span data-ttu-id="93ded-256">添加全局变量和文档**就绪**事件处理程序。</span><span class="sxs-lookup"><span data-stu-id="93ded-256">Add the global variables and the document **ready** event handler.</span></span> <span data-ttu-id="93ded-257">可以使用 jQuery 函数访问**document**对象。</span><span class="sxs-lookup"><span data-stu-id="93ded-257">The **document** object is accessed by using a jQuery function.</span></span> 
...
### <a name="adding-a-ribbon-custom-action"></a><span data-ttu-id="93ded-293">添加功能区自定义操作</span><span class="sxs-lookup"><span data-stu-id="93ded-293">Adding a ribbon custom action</span></span>
...
2. <span data-ttu-id="93ded-312">在 Visual Studio 中的 "**解决方案资源管理器**" 窗格中，右键单击 " **QuickStatus** " 项目，然后添加新项目。</span><span class="sxs-lookup"><span data-stu-id="93ded-312">In the **Solution Explorer** pane in Visual Studio, right-click the **QuickStatus** project, and then add a new item.</span></span> <span data-ttu-id="93ded-313">在 "**添加新项**" 对话框中，选择 "**功能区自定义操作**" （请参阅图4）。</span><span class="sxs-lookup"><span data-stu-id="93ded-313">In the **Add New Item** dialog box, choose **Ribbon Custom Action** (see Figure 4).</span></span> <span data-ttu-id="93ded-314">例如，将自定义操作命名为 RibbonQuickStatusAction，然后选择 "**添加**"。</span><span class="sxs-lookup"><span data-stu-id="93ded-314">For example, name the custom action RibbonQuickStatusAction, and then choose **Add**.</span></span>
   ...
   <span data-ttu-id="93ded-316">![添加功能区自定义操作](media/pj15_CreateStatusingApp_AddRibbonCustomAction.gif "添加功能区自定义操作")</span><span class="sxs-lookup"><span data-stu-id="93ded-316">![Adding a ribbon custom action](media/pj15_CreateStatusingApp_AddRibbonCustomAction.gif "Adding a ribbon custom action")</span></span>
  
3. <span data-ttu-id="93ded-317">在 "**创建面向功能区的自定义操作**向导" 的第一页上，保持 "**主机 Web** " 选项处于选中状态，在自定义操作范围的下拉列表中选择 "**无**"，然后选择 "**下一步**" （请参阅图5）。</span><span class="sxs-lookup"><span data-stu-id="93ded-317">On the first page of the **Create Custom Action for Ribbon** wizard, leave the **Host Web** option selected, choose **None** in the drop-down list for the custom action scope, and then choose **Next** (see Figure 5).</span></span> <span data-ttu-id="93ded-318">下拉列表中的项与 SharePoint 相关，而不适用于 Project Server。</span><span class="sxs-lookup"><span data-stu-id="93ded-318">The items in the drop-down lists are relevant to SharePoint, not to Project Server.</span></span> <span data-ttu-id="93ded-319">我们将把大部分生成的 XML 替换为自定义操作，使其适用于 Project Server。</span><span class="sxs-lookup"><span data-stu-id="93ded-319">We will replace most of the generated XML for the custom action so that it applies to Project Server.</span></span> 
    
   <span data-ttu-id="93ded-320">**图5。指定功能区自定义操作的属性**</span><span class="sxs-lookup"><span data-stu-id="93ded-320">**Figure 5. Specifying properties for the ribbon custom action**</span></span>

   <span data-ttu-id="93ded-321">![指定功能区自定义操作的属性](media/pj15_CreateStatusingApp_RibbonCustomAction2.gif "指定功能区自定义操作的属性")</span><span class="sxs-lookup"><span data-stu-id="93ded-321">![Specifying properties for the ribbon custom action](media/pj15_CreateStatusingApp_RibbonCustomAction2.gif "Specifying properties for the ribbon custom action")</span></span>
  
4. <span data-ttu-id="93ded-322">在 "**创建功能区的自定义操作**向导" 的下一页上，保留设置的所有默认值，然后选择 "**完成**" （请参阅图6）。</span><span class="sxs-lookup"><span data-stu-id="93ded-322">On the next page of the **Create Custom Action for Ribbon** wizard, leave all the default values for the settings, and then choose **Finish** (see Figure 6).</span></span> <span data-ttu-id="93ded-323">Visual Studio 创建**RibbonQuickStatusAction**文件夹，其中包含一个元素 .xml 文件。</span><span class="sxs-lookup"><span data-stu-id="93ded-323">Visual Studio creates the **RibbonQuickStatusAction** folder, which contains an Elements.xml file.</span></span> 
    
   <span data-ttu-id="93ded-324">**图6。指定按钮控件的设置**</span><span class="sxs-lookup"><span data-stu-id="93ded-324">**Figure 6. Specifying the settings for a button control**</span></span>

   <span data-ttu-id="93ded-325">![指定按钮控件的设置](media/pj15_CreateStatusingApp_RibbonCustomAction3.gif "指定按钮控件的设置")</span><span class="sxs-lookup"><span data-stu-id="93ded-325">![Specifying the settings for a button control](media/pj15_CreateStatusingApp_RibbonCustomAction3.gif "Specifying the settings for a button control")</span></span>
  
5. <span data-ttu-id="93ded-326">修改功能区自定义操作的元素 .xml 文件中的默认生成代码。</span><span class="sxs-lookup"><span data-stu-id="93ded-326">Modify the default generated code in the Elements.xml file for the ribbon custom action.</span></span> <span data-ttu-id="93ded-327">以下是默认的 XML 代码：</span><span class="sxs-lookup"><span data-stu-id="93ded-327">Following is the default XML code:</span></span>
...
<span data-ttu-id="93ded-348">在开发应用程序并添加功能区按钮时，通常会测试应用并在 JavaScript 代码中设置断点以进行调试。</span><span class="sxs-lookup"><span data-stu-id="93ded-348">As you develop the app and add a ribbon button, you normally test the app and set breakpoints in the JavaScript code for debugging.</span></span> <span data-ttu-id="93ded-349">当您按**F5**启动调试时，Visual Studio 会编译应用程序，将其部署到在**QuickStatus**项目的 "**网站 URL** " 属性中指定的网站，并显示一页，询问您是否信任该应用程序。</span><span class="sxs-lookup"><span data-stu-id="93ded-349">When you press **F5** to start debugging, Visual Studio compiles the app, deploys it to the site that is specified in the **Site URL** property of the **QuickStatus** project, and displays a page that asks whether you trust the app.</span></span> <span data-ttu-id="93ded-350">在继续操作后退出**QuickStatus**应用程序时，它会返回到 Project Web app 中的任务页面。</span><span class="sxs-lookup"><span data-stu-id="93ded-350">When you proceed and then exit the **QuickStatus** app, it returns to the Tasks page in Project Web App.</span></span> 

> [!NOTE]
> <span data-ttu-id="93ded-351">图7显示功能区的 "**任务**" 选项卡上的 "**快速状态**" 按钮已禁用。</span><span class="sxs-lookup"><span data-stu-id="93ded-351">Figure 7 shows that the **Quick Status** button on the **TASKS** tab of the ribbon is disabled.</span></span> <span data-ttu-id="93ded-352">在使用 Visual Studio 进行许多调试部署后，在同一测试服务器上继续调试或部署已发布的应用程序时，可能会阻止自定义功能区控件。</span><span class="sxs-lookup"><span data-stu-id="93ded-352">After many debug deployments with Visual Studio, custom ribbon controls can be blocked when you continue to debug or deploy the published app on the same test server.</span></span> <span data-ttu-id="93ded-353">若要启用按钮，请删除 Visual Studio 中的**RibbonQuickStatusAction**项，然后创建一个具有不同名称和 ID 的新的功能区操作。</span><span class="sxs-lookup"><span data-stu-id="93ded-353">To enable the button, delete the **RibbonQuickStatusAction** item in Visual Studio, and then create a new ribbon action that has a different name and ID.</span></span> <span data-ttu-id="93ded-354">如果这不能解决问题，请尝试从 Project Web App 测试实例中删除应用程序，然后使用不同的应用 ID 重新创建应用。</span><span class="sxs-lookup"><span data-stu-id="93ded-354">If that doesn't solve the problem, try removing the app from the Project Web App test instance, and then recreate the app with a different app ID.</span></span> 
  
<span data-ttu-id="93ded-355">**图7。查看已禁用的快速状态按钮的工具提示**</span><span class="sxs-lookup"><span data-stu-id="93ded-355">**Figure 7. Viewing the tooltip of the disabled Quick Status button**</span></span>

<span data-ttu-id="93ded-356">![查看禁用按钮的工具提示](media/pj15_CreateStatusingApp_ButtonToolTipDisabled.gif "查看禁用按钮的工具提示")</span><span class="sxs-lookup"><span data-stu-id="93ded-356">![Viewing the tooltip of the disabled button](media/pj15_CreateStatusingApp_ButtonToolTipDisabled.gif "Viewing the tooltip of the disabled button")</span></span>
  
<span data-ttu-id="93ded-357">步骤5显示了如何部署和安装**QuickStatus**应用程序。</span><span class="sxs-lookup"><span data-stu-id="93ded-357">Procedure 5 shows how to deploy and install the **QuickStatus** app.</span></span> <span data-ttu-id="93ded-358">步骤6显示了在安装应用程序后测试该应用程序的一些额外步骤。</span><span class="sxs-lookup"><span data-stu-id="93ded-358">Procedure 6 shows some additional steps in testing the app after you have installed it.</span></span> 

<span data-ttu-id="93ded-359"><a name="pj15_StatusingApp_Deploying"> </a></span><span class="sxs-lookup"><span data-stu-id="93ded-359"><a name="pj15_StatusingApp_Deploying"> </a></span></span>

## <a name="deploying-the-quickstatus-app"></a><span data-ttu-id="93ded-360">部署 QuickStatus 应用程序</span><span class="sxs-lookup"><span data-stu-id="93ded-360">Deploying the QuickStatus app</span></span>

<span data-ttu-id="93ded-361">有几种方法可将应用程序部署到 SharePoint web 应用程序（如 Project Web App）。</span><span class="sxs-lookup"><span data-stu-id="93ded-361">There are several ways to deploy an app to a SharePoint web application such as Project Web App.</span></span> <span data-ttu-id="93ded-362">您使用哪种部署取决于您想要将应用程序发布到专用 SharePoint 目录还是发布到公共 Office 商店，以及 SharePoint 是安装在本地还是在线租赁。</span><span class="sxs-lookup"><span data-stu-id="93ded-362">Which deployment you use will depend on whether you want to publish the app to a private SharePoint catalog or to the public Office Store, and whether SharePoint is installed on-premises or is an online tenancy.</span></span> <span data-ttu-id="93ded-363">步骤5显示了如何将**QuickStatus**应用程序部署到私有应用程序目录中的本地安装。</span><span class="sxs-lookup"><span data-stu-id="93ded-363">Procedure 5 shows how to deploy the **QuickStatus** app to an on-premises installation in a private app catalog.</span></span> <span data-ttu-id="93ded-364">有关详细信息，请参阅[安装和管理 sharepoint 相关应用程序 2013](https://technet.microsoft.com/library/fp161232.aspx)和[发布 sharepoint 相关应用](https://msdn.microsoft.com/library/jj164070.aspx)程序</span><span class="sxs-lookup"><span data-stu-id="93ded-364">For more information, see [Install and manage apps for SharePoint 2013](https://technet.microsoft.com/library/fp161232.aspx) and [Publish apps for SharePoint](https://msdn.microsoft.com/library/jj164070.aspx)</span></span>
  
> [!NOTE]
> <span data-ttu-id="93ded-365">将应用程序添加到 SharePoint 目录需要 SharePoint 管理员权限。</span><span class="sxs-lookup"><span data-stu-id="93ded-365">Adding an app to a SharePoint catalog requires SharePoint administrator permissions.</span></span> 
  
### <a name="procedure-5-to-deploy-the-quickstatus-app"></a><span data-ttu-id="93ded-366">过程 5.</span><span class="sxs-lookup"><span data-stu-id="93ded-366">Procedure 5.</span></span> <span data-ttu-id="93ded-367">部署 QuickStatus 应用程序</span><span class="sxs-lookup"><span data-stu-id="93ded-367">To deploy the QuickStatus app</span></span>

1. <span data-ttu-id="93ded-368">在 Visual Studio 中，保存所有文件，然后在 "**解决方案资源管理器**" 中右键单击 " **QuickStatus** " 项目，然后选择 "**发布**"。</span><span class="sxs-lookup"><span data-stu-id="93ded-368">In Visual Studio, save all of the files, and then right-click the **QuickStatus** project in the **Solution Explorer** and choose **Publish**.</span></span>
    
2. <span data-ttu-id="93ded-369">由于**QuickStatus**应用程序是 SharePoint 托管的，因此发布的选项非常少（请参阅图8）。</span><span class="sxs-lookup"><span data-stu-id="93ded-369">Because the **QuickStatus** app is SharePoint-hosted, there are very few options for publishing (see Figure 8).</span></span> <span data-ttu-id="93ded-370">在 "**发布 Office 和 SharePoint 相关应用程序**" 对话框中，选择 "**完成**"。</span><span class="sxs-lookup"><span data-stu-id="93ded-370">In the **Publish apps for Office and SharePoint** dialog box, choose **Finish**.</span></span>
    
   <span data-ttu-id="93ded-371">**图8。发布 QuickStatus 应用程序**</span><span class="sxs-lookup"><span data-stu-id="93ded-371">**Figure 8. Publishing the QuickStatus app**</span></span>

   <span data-ttu-id="93ded-372">![使用发布向导](media/pj15_CreateStatusingApp_PublishWizard.gif "使用发布向导")</span><span class="sxs-lookup"><span data-stu-id="93ded-372">![Using the Publish Wizard](media/pj15_CreateStatusingApp_PublishWizard.gif "Using the Publish Wizard")</span></span>
  
3. <span data-ttu-id="93ded-373">将 QuickStatus 文件从`~\QuickStatus\bin\Debug\app.publish\1.0.0.0`目录复制到本地计算机上的方便目录（或本地安装的 SharePoint 计算机）。</span><span class="sxs-lookup"><span data-stu-id="93ded-373">Copy the QuickStatus.app file from the  `~\QuickStatus\bin\Debug\app.publish\1.0.0.0` directory to a convenient directory on the local computer (or to the SharePoint computer for an on-premises installation).</span></span> 
    
4. <span data-ttu-id="93ded-374">在 SharePoint 管理中心中，选择 "快速启动" 中的 "**应用程序**"，然后选择 "**管理应用程序目录**"。</span><span class="sxs-lookup"><span data-stu-id="93ded-374">In SharePoint Central Administration, choose **Apps** in the Quick Launch, and then choose **Manage App Catalog**.</span></span>
    
5. <span data-ttu-id="93ded-375">如果应用程序目录不存在，请按照在[SharePoint 2013 中管理应用程序目录](https://technet.microsoft.com/library/fp161234.aspx)中的 "*为 Web 应用程序配置应用程序目录网站*" 部分，为应用程序目录创建网站集。</span><span class="sxs-lookup"><span data-stu-id="93ded-375">If an app catalog does not exist, create a site collection for the app catalog, by following the  *Configure the App Catalog site for a web application*  section in [Manage the App Catalog in SharePoint 2013](https://technet.microsoft.com/library/fp161234.aspx).</span></span>
    
   <span data-ttu-id="93ded-376">如果存在应用程序目录，则导航到 "管理应用程序目录" 页上的 "网站 URL"。</span><span class="sxs-lookup"><span data-stu-id="93ded-376">If an app catalog exists, navigate to the site URL on the Manage App Catalog page.</span></span> <span data-ttu-id="93ded-377">例如，在以下步骤中，应用程序目录网站为`https://ServerName/sites/TestApps`。</span><span class="sxs-lookup"><span data-stu-id="93ded-377">For example, in the following steps, the app catalog site is  `https://ServerName/sites/TestApps`.</span></span>
    
6. <span data-ttu-id="93ded-378">在 "应用程序目录" 页上，选择 "快速启动" 中的 " **SharePoint 相关应用程序**"。</span><span class="sxs-lookup"><span data-stu-id="93ded-378">On the app catalog page, choose **Apps for SharePoint** in the Quick Launch.</span></span> <span data-ttu-id="93ded-379">在 "SharePoint 相关应用程序" 页上，在功能区的 "**文件**" 选项卡上，选择 "**上载文档**"。</span><span class="sxs-lookup"><span data-stu-id="93ded-379">On the Apps for SharePoint page, on the **FILES** tab of the ribbon, choose **Upload Document**.</span></span>
    
7. <span data-ttu-id="93ded-380">在 "**添加文档**" 对话框中，浏览 QuickStatus 文件，为版本添加注释，然后选择 **"确定"**。</span><span class="sxs-lookup"><span data-stu-id="93ded-380">In the **Add a document** dialog box, browse for the QuickStatus.app file, add comments for the version, and then choose **OK**.</span></span>
    
8. <span data-ttu-id="93ded-381">添加应用程序时，您还可以添加应用程序说明、图标和其他信息的本地信息。</span><span class="sxs-lookup"><span data-stu-id="93ded-381">When you add an app, you can also add local information for the app description, icon, and other information.</span></span> <span data-ttu-id="93ded-382">在 " **Sharepoint 相关应用程序-QuickStatus** " 对话框中，添加要在 sharepoint 网站集中为该应用程序显示的信息。</span><span class="sxs-lookup"><span data-stu-id="93ded-382">In the **Apps for SharePoint - QuickStatus.app** dialog box, add the information that you want to show for the app in the SharePoint site collection.</span></span> <span data-ttu-id="93ded-383">例如，添加以下信息：</span><span class="sxs-lookup"><span data-stu-id="93ded-383">For example, add the following information:</span></span> 
    
   1. <span data-ttu-id="93ded-384">**短说明**字段：键入快速状态测试应用。</span><span class="sxs-lookup"><span data-stu-id="93ded-384">**Short Description** field: Type Quick Status test app.</span></span>
    
   2. <span data-ttu-id="93ded-385">**说明**字段：键入测试应用程序以更新多个项目中的任务的完成百分比。</span><span class="sxs-lookup"><span data-stu-id="93ded-385">**Description** field: Type Test app to update percent complete for tasks in multiple projects.</span></span>
    
   3. <span data-ttu-id="93ded-386">**图标 URL**字段：将应用程序图标的 96 x 96 像素图像添加到应用程序目录的网站资产中。</span><span class="sxs-lookup"><span data-stu-id="93ded-386">**Icon URL** fields: Add a 96 x 96-pixel image for the app icon to the site assets for the app catalog.</span></span> <span data-ttu-id="93ded-387">例如，导航到`https://ServerName/sites/TestApps`，选择 "**设置**" 下拉菜单中的 "**网站内容**"，选择 "**网站资产**"，然后添加 "quickStatusApp" 图像。</span><span class="sxs-lookup"><span data-stu-id="93ded-387">For example, navigate to  `https://ServerName/sites/TestApps`, choose **Site contents** in the **Settings** drop-down menu, choose **Site Assets**, and then add the quickStatusApp.png image.</span></span> <span data-ttu-id="93ded-388">右键单击 " **quickStatusApp** " 项目，选择 "**属性**"，然后在 "**属性**" 对话框中复制 "**地址（URL）** " 值。</span><span class="sxs-lookup"><span data-stu-id="93ded-388">Right-click the **quickStatusApp** item, choose **Properties**, and then copy the **Address (URL)** value in the **Properties** dialog box.</span></span> <span data-ttu-id="93ded-389">例如，复制`https://ServerName/sites/TestApps/SiteAssets/QuickStatusApp.png`并粘贴 "**图标 URL** web 地址" 字段中的值。</span><span class="sxs-lookup"><span data-stu-id="93ded-389">For example, copy  `https://ServerName/sites/TestApps/SiteAssets/QuickStatusApp.png`, and then paste the value in the **Icon URL** web address field.</span></span> <span data-ttu-id="93ded-390">键入图标的说明（例如，如图9所示），键入 QuickStatus app 图标。</span><span class="sxs-lookup"><span data-stu-id="93ded-390">Type a description for the icon, for example (as in Figure 9), type QuickStatus app icon.</span></span> <span data-ttu-id="93ded-391">测试 URL 是否有效。</span><span class="sxs-lookup"><span data-stu-id="93ded-391">Test that the URL is valid.</span></span>
    
      <span data-ttu-id="93ded-392">**图9。为 QuickStatus 应用程序添加图标 URL**</span><span class="sxs-lookup"><span data-stu-id="93ded-392">**Figure 9. Adding an icon URL for the QuickStatus app**</span></span>

      <span data-ttu-id="93ded-393">![在 SharePoint 中为应用程序设置属性](media/pj15_CreateStatusingApp_AddAppToSharePointSettings.gif "在 SharePoint 中为应用程序设置属性")</span><span class="sxs-lookup"><span data-stu-id="93ded-393">![Setting properties in SharePoint for the app](media/pj15_CreateStatusingApp_AddAppToSharePointSettings.gif "Setting properties in SharePoint for the app")</span></span>
  
   4. <span data-ttu-id="93ded-394">**类别**字段：选择现有的类别，或指定自己的值。</span><span class="sxs-lookup"><span data-stu-id="93ded-394">**Category** field: Choose an existing category, or specify your own value.</span></span> <span data-ttu-id="93ded-395">例如，键入 Statusing。</span><span class="sxs-lookup"><span data-stu-id="93ded-395">For example, type Statusing.</span></span>
    
      > [!NOTE]
      > <span data-ttu-id="93ded-396">名为 " **Statusing** " 的类别仅用于测试目的。</span><span class="sxs-lookup"><span data-stu-id="93ded-396">A category named **Statusing** is just for testing purposes.</span></span> <span data-ttu-id="93ded-397">Project Server 应用程序的典型类别是**项目管理**。</span><span class="sxs-lookup"><span data-stu-id="93ded-397">A typical category for Project Server apps is **Project Management**.</span></span> 
  
   5. <span data-ttu-id="93ded-398">"**发布者名称**" 字段：键入发布服务器的名称。</span><span class="sxs-lookup"><span data-stu-id="93ded-398">**Publisher name** field: Type the name of the publisher.</span></span> <span data-ttu-id="93ded-399">在此示例中，键入 Project SDK。</span><span class="sxs-lookup"><span data-stu-id="93ded-399">In this example, type Project SDK.</span></span>
    
   6. <span data-ttu-id="93ded-400">**启用**的字段：若要使应用程序对 Project Web app 网站管理员的安装可见，请选中 "**已启用**" 复选框。</span><span class="sxs-lookup"><span data-stu-id="93ded-400">**Enabled** field: To make the app visible to Project Web App site administrators for installation, select the **Enabled** check box.</span></span> 
    
   7. <span data-ttu-id="93ded-401">其他字段是可选的。</span><span class="sxs-lookup"><span data-stu-id="93ded-401">Additional fields are optional.</span></span> <span data-ttu-id="93ded-402">例如，您可以为应用程序详细信息页添加支持 URL 和多个帮助图像。</span><span class="sxs-lookup"><span data-stu-id="93ded-402">For example, you can add a support URL and multiple help images for the app details page.</span></span> <span data-ttu-id="93ded-403">图9中的 "**图像 URL 1** " 字段包含应用程序屏幕截图的 URL 和屏幕截图的说明。</span><span class="sxs-lookup"><span data-stu-id="93ded-403">In Figure 9, the **Image URL 1** fields include the URL for a screenshot of the app and a description of the screenshot.</span></span> 
    
   8. <span data-ttu-id="93ded-404">在 " **SharePoint 相关应用程序-QuickStatus** " 对话框中，选择 "**保存**"。</span><span class="sxs-lookup"><span data-stu-id="93ded-404">In the **Apps for SharePoint - QuickStatus.app** dialog box, choose **Save**.</span></span> <span data-ttu-id="93ded-405">图9中的 "SharePoint 相关应用程序库中的**快速状态更新**" 项已签出，因此在对话框功能区的 "**编辑**" 选项卡上，选择 "**签入**" 以完成此过程（见图10）。</span><span class="sxs-lookup"><span data-stu-id="93ded-405">In Figure 9, the **Quick Status Update** item in the Apps for SharePoint library is checked out for editing, so on the **EDIT** tab of the dialog box ribbon, you would choose **Check In** to complete the process (see Figure 10).</span></span> 
    
      <span data-ttu-id="93ded-406">**图10。将 QuickStatus 应用程序添加到 SharePoint 相关应用程序库。**</span><span class="sxs-lookup"><span data-stu-id="93ded-406">**Figure 10. The QuickStatus app is added to the Apps for SharePoint library.**</span></span>

      <span data-ttu-id="93ded-407">![将 QuickStatus 应用程序添加到 SharePoint](media/pj15_CreateStatusingApp_AddAppToSharePoint.gif "将 QuickStatus 应用程序添加到 SharePoint")</span><span class="sxs-lookup"><span data-stu-id="93ded-407">![The QuickStatus app is added to SharePoint](media/pj15_CreateStatusingApp_AddAppToSharePoint.gif "The QuickStatus app is added to SharePoint")</span></span>
  
9. <span data-ttu-id="93ded-408">在 Project Web App 的 "**设置**" 下拉菜单中，选择 "**添加应用程序**"。</span><span class="sxs-lookup"><span data-stu-id="93ded-408">In Project Web App, in the **Settings** drop-down menu, choose **Add an app**.</span></span> <span data-ttu-id="93ded-409">在 "应用程序" 页上的 "快速启动" 中，选择 "**来自您的组织**"，然后选择 "**快速状态更新**" 应用程序的 "**应用程序详细信息**"。</span><span class="sxs-lookup"><span data-stu-id="93ded-409">On the Your Apps page, in the Quick Launch, choose **From Your Organization**, and then choose **App Details** for the **Quick Status Update** app.</span></span> <span data-ttu-id="93ded-410">图11显示了带有在上一步中添加的应用程序图标、屏幕截图和其他信息的详细信息页。</span><span class="sxs-lookup"><span data-stu-id="93ded-410">Figure 11 shows the details page with the app icon, screenshot, and other information that you added in the previous step.</span></span> 
    
   <span data-ttu-id="93ded-411">**图11。使用 Project Web App 中的 "快速状态更新详细信息" 页**</span><span class="sxs-lookup"><span data-stu-id="93ded-411">**Figure 11. Using the Quick Status Update details page in Project Web App**</span></span>

   <span data-ttu-id="93ded-412">![将 QuickStatus 应用程序添加到 Project Web App](media/pj15_CreateStatusingApp_AddAppToPWA.gif "将 QuickStatus 应用程序添加到 Project Web App")</span><span class="sxs-lookup"><span data-stu-id="93ded-412">![Adding the QuickStatus app to Project Web App](media/pj15_CreateStatusingApp_AddAppToPWA.gif "Adding the QuickStatus app to Project Web App")</span></span>
  
10. <span data-ttu-id="93ded-413">在 "快速状态更新详细信息" 页上，选择 "**添加**"。</span><span class="sxs-lookup"><span data-stu-id="93ded-413">On the Quick Status Update details page, choose **ADD IT**.</span></span> <span data-ttu-id="93ded-414">Project Web App 显示一个对话框，其中列出了 QuickStatus 应用程序可以执行的操作（见图12）。</span><span class="sxs-lookup"><span data-stu-id="93ded-414">Project Web App displays a dialog box that lists the operations that the QuickStatus app can perform (see Figure 12).</span></span> <span data-ttu-id="93ded-415">操作列表派生自 Appmanifest.xml 文件中的**AppPermissionRequest**元素。</span><span class="sxs-lookup"><span data-stu-id="93ded-415">The list of operations is derived from the **AppPermissionRequest** elements in the AppManifest.xml file.</span></span> 
    
    <span data-ttu-id="93ded-416">**图12。验证您是否信任 "快速状态" 应用程序**</span><span class="sxs-lookup"><span data-stu-id="93ded-416">**Figure 12. Verifying that you trust the Quick Status app**</span></span>

    <span data-ttu-id="93ded-417">![验证对 QuickStatus 应用程序的信任](media/pj15_CreateStatusingApp_AddAppToPWA2Trust.gif "验证对 QuickStatus 应用程序的信任")</span><span class="sxs-lookup"><span data-stu-id="93ded-417">![Verifying trust for the QuickStatus app](media/pj15_CreateStatusingApp_AddAppToPWA2Trust.gif "Verifying trust for the QuickStatus app")</span></span>
  
11. <span data-ttu-id="93ded-418">在 "**是否信任快速状态更新**" 对话框中，选择 "**信任它**"。</span><span class="sxs-lookup"><span data-stu-id="93ded-418">In the **Do you trust Quick Status Update** dialog box, choose **Trust It**.</span></span> <span data-ttu-id="93ded-419">该应用程序将添加到 Project Web App "网站内容" 页面（见图13）。</span><span class="sxs-lookup"><span data-stu-id="93ded-419">The app is added to the Project Web App Site Contents page (see Figure 13).</span></span>
    
    <span data-ttu-id="93ded-420">**图13。查看 "网站内容" 页上的 "快速状态" 应用程序**</span><span class="sxs-lookup"><span data-stu-id="93ded-420">**Figure 13. Viewing the Quick Status app on the Site Contents page**</span></span>

    <span data-ttu-id="93ded-421">![在“网站内容”中查看 QuickStatus 应用程序](media/pj15_CreateStatusingApp_AddAppToPWA3.gif "在“网站内容”中查看 QuickStatus 应用程序")</span><span class="sxs-lookup"><span data-stu-id="93ded-421">![Viewing the QuickStatus app in Site Contents](media/pj15_CreateStatusingApp_AddAppToPWA3.gif "Viewing the QuickStatus app in Site Contents")</span></span>
  
<span data-ttu-id="93ded-422">在 "网站内容" 页上，您可以选择 "**快速状态更新**" 图标以运行应用程序。</span><span class="sxs-lookup"><span data-stu-id="93ded-422">On the Site Contents page, you can select the **Quick Status Update** icon to run the app.</span></span>

> [!NOTE]
> <span data-ttu-id="93ded-423">有关提供有关应用程序的信息的其他命令，请在 "网站内容" 页上，选择包含 "**快速状态更新**" 名称和省略号（"..."）的区域。您可以查看应用程序的 "关于" 页、查看包含应用程序错误信息的 "应用程序详细信息" 页、查看 "应用程序权限" 页或从 Project Web App 中删除该应用程序。</span><span class="sxs-lookup"><span data-stu-id="93ded-423">For additional commands that provide information about the app, on the Site Contents page, choose the region that contains the **Quick Status Update** name and the ellipsis (...). You can review the About page for the app, view the App Details page that contains information about app errors, review the app permissions page, or remove the app from Project Web App.</span></span> 
  
<span data-ttu-id="93ded-424">在 Project Web App 的 "任务" 页上（见图14），应在功能区上启用 " **QuickStatus** " 按钮。</span><span class="sxs-lookup"><span data-stu-id="93ded-424">On the Tasks page in Project Web App (see Figure 14), the **QuickStatus** button should be enabled on the ribbon.</span></span> <span data-ttu-id="93ded-425">如果 "**快速状态**" 按钮处于禁用状态，请尝试图7的注释中所述的操作。</span><span class="sxs-lookup"><span data-stu-id="93ded-425">If the **Quick Status** button is disabled, try the actions described in the note for Figure 7.</span></span> 

<span data-ttu-id="93ded-426">**图14。从 "任务" 选项卡启动 QuickStatus 应用程序**</span><span class="sxs-lookup"><span data-stu-id="93ded-426">**Figure 14. Starting the QuickStatus app from the TASKS tab**</span></span>

<span data-ttu-id="93ded-427">![从“任务”选项卡启动 QuickStatus 应用程序](media/pj15_CreateStatusingApp_TasksRibbon.gif "从“任务”选项卡启动 QuickStatus 应用程序")</span><span class="sxs-lookup"><span data-stu-id="93ded-427">![Starting the QuickStatus app from the TASKS tab](media/pj15_CreateStatusingApp_TasksRibbon.gif "Starting the QuickStatus app from the TASKS tab")</span></span>
  
<span data-ttu-id="93ded-428">过程6显示了使用 QuickStatus 应用程序进行的一些测试。</span><span class="sxs-lookup"><span data-stu-id="93ded-428">Procedure 6 shows some tests to make with the QuickStatus app.</span></span>

<span data-ttu-id="93ded-429"><a name="pj15_StatusingApp_Testing"> </a></span><span class="sxs-lookup"><span data-stu-id="93ded-429"><a name="pj15_StatusingApp_Testing"> </a></span></span>

## <a name="testing-the-quickstatus-app"></a><span data-ttu-id="93ded-430">测试 QuickStatus 应用程序</span><span class="sxs-lookup"><span data-stu-id="93ded-430">Testing the QuickStatus app</span></span>

<span data-ttu-id="93ded-431">在将应用程序部署到生产服务器或 Project Online 的生产租户之前，应在 Project Server 的测试安装上测试用户在**QuickStatus**应用程序中可能尝试的每个操作。</span><span class="sxs-lookup"><span data-stu-id="93ded-431">Every operation that a user might try in the **QuickStatus** app should be tested on a test installation of Project Server before deploying the app to a production server or to a production tenant of Project Online.</span></span> <span data-ttu-id="93ded-432">通过测试安装，您可以更改和删除用户的工作分配，而不会影响实际项目。</span><span class="sxs-lookup"><span data-stu-id="93ded-432">A test installation enables you to change and delete assignments for users without affecting actual projects.</span></span> <span data-ttu-id="93ded-433">测试还应涉及几个具有不同权限集的用户，如管理员、项目经理和团队成员。</span><span class="sxs-lookup"><span data-stu-id="93ded-433">Testing should also involve several users who have different sets of permissions, such as administrator, project manager, and team member.</span></span> <span data-ttu-id="93ded-434">全面测试可以发现应在应用程序中进行的更改，在开发过程中测试不明显。</span><span class="sxs-lookup"><span data-stu-id="93ded-434">Thorough testing can uncover changes that should be made in the app, which were not apparent in testing during development.</span></span> <span data-ttu-id="93ded-435">过程6列出了**QuickStatus**应用程序的多个测试，但不包括一系列详尽的测试。</span><span class="sxs-lookup"><span data-stu-id="93ded-435">Procedure 6 lists several tests for the **QuickStatus** app, but does not include an exhaustive series of tests.</span></span> 
  
### <a name="procedure-6-to-test-the-quickstatus-app"></a><span data-ttu-id="93ded-436">过程 6.</span><span class="sxs-lookup"><span data-stu-id="93ded-436">Procedure 6.</span></span> <span data-ttu-id="93ded-437">测试 QuickStatus 应用程序</span><span class="sxs-lookup"><span data-stu-id="93ded-437">To test the QuickStatus app</span></span>

1. <span data-ttu-id="93ded-438">运行用户没有工作分配的**QuickStatus**应用程序。</span><span class="sxs-lookup"><span data-stu-id="93ded-438">Run the **QuickStatus** app where the user has no assignments.</span></span> <span data-ttu-id="93ded-439">应用程序应在页面底部显示一条蓝色消息，例如，"用户名"**没有分配**。</span><span class="sxs-lookup"><span data-stu-id="93ded-439">The app should show a blue message at the bottom of the page, for example, **User Name has no assignments**.</span></span>
    
   <span data-ttu-id="93ded-440">选择 "**更新**"，并更新对绿色**分配**的邮件所做的更改。</span><span class="sxs-lookup"><span data-stu-id="93ded-440">Choose **Update**, and the message changes to a green **Assignments have been updated**.</span></span>
    
   > [!NOTE]
   > <span data-ttu-id="93ded-441">应更改应用程序行为，以便在没有工作分配时禁用 "**更新**" 按钮。</span><span class="sxs-lookup"><span data-stu-id="93ded-441">The app behavior should be changed so that the **Update** button is disabled when there are no assignments.</span></span> 
  
2. <span data-ttu-id="93ded-442">运行用户在多个不同的项目中有多个分配的应用程序，并且一些工作分配不完整。</span><span class="sxs-lookup"><span data-stu-id="93ded-442">Run the app where the user has multiple assignments in several different projects and some assignments are not complete.</span></span> <span data-ttu-id="93ded-443">请注意应用程序的外观，并按如下所示执行操作（请参阅图15）：</span><span class="sxs-lookup"><span data-stu-id="93ded-443">Notice the appearance of the app and perform actions as follows (see Figure 15):</span></span>
    
   1. <span data-ttu-id="93ded-444">**OnGetAssignmentsSuccess**函数为当前用户的每个分配在表中创建一行。</span><span class="sxs-lookup"><span data-stu-id="93ded-444">The **onGetAssignmentsSuccess** function creates a row in the table for each assignment for the current user.</span></span> <span data-ttu-id="93ded-445">项目名称仅以粗体显示一次，每个项目中的第一个工作分配。</span><span class="sxs-lookup"><span data-stu-id="93ded-445">The project name shows only once, in a bold font, for the first assignment in each project.</span></span> 
    
   2. <span data-ttu-id="93ded-446">清除 "**任务名称**" 列标题中的复选框。</span><span class="sxs-lookup"><span data-stu-id="93ded-446">Clear the check box in the **Task name** column header.</span></span> <span data-ttu-id="93ded-447">表头单击事件处理程序将清除任务行中的所有其他复选框。</span><span class="sxs-lookup"><span data-stu-id="93ded-447">The table header click event handler clears all of the other check boxes in the task rows.</span></span> 
    
   3. <span data-ttu-id="93ded-448">选择所有任务。</span><span class="sxs-lookup"><span data-stu-id="93ded-448">Select all of the tasks.</span></span> <span data-ttu-id="93ded-449">每个行的 click 事件处理程序确定是否选择了所有行，如果是，则选择 "**任务名称**" 列标题。</span><span class="sxs-lookup"><span data-stu-id="93ded-449">The click event handler for each row determines whether all rows are selected, and if so, selects the **Task name** column header.</span></span> 
    
   4. <span data-ttu-id="93ded-450">再次清除所有复选框，然后选择一个具有剩余工时的工作分配。</span><span class="sxs-lookup"><span data-stu-id="93ded-450">Clear all of the check boxes again, and then select one assignment that has some remaining work.</span></span> <span data-ttu-id="93ded-451">例如，图15显示了首要任务 T1 的剩余20% 的工作要完成。</span><span class="sxs-lookup"><span data-stu-id="93ded-451">For example, Figure 15 shows the top task T1 has 20% remaining work to complete.</span></span>
    
   5. <span data-ttu-id="93ded-452">在 "**设置完成百分比**" 文本框中，键入80，然后选择 "**更新**"。</span><span class="sxs-lookup"><span data-stu-id="93ded-452">In the **Set percent complete** text box, type 80, and then choose **Update**.</span></span> <span data-ttu-id="93ded-453">页面底部应显示绿色消息，已**更新分配**。</span><span class="sxs-lookup"><span data-stu-id="93ded-453">The bottom of the page should show a green message, **Assignments have been updated**.</span></span>
    
      <span data-ttu-id="93ded-454">**图15。更新 QuickStatus 应用程序中的工作分配**</span><span class="sxs-lookup"><span data-stu-id="93ded-454">**Figure 15. Updating an assignment in the QuickStatus app**</span></span>

      <span data-ttu-id="93ded-455">![更新 QuickStatus 应用程序中的工作分配](media/pj15_CreateStatusingApp_Testing1Update.gif "更新 QuickStatus 应用程序中的工作分配")</span><span class="sxs-lookup"><span data-stu-id="93ded-455">![Updating an assignment in the QuickStatus app](media/pj15_CreateStatusingApp_Testing1Update.gif "Updating an assignment in the QuickStatus app")</span></span>
  
3. <span data-ttu-id="93ded-456">选择 "**刷新**" （见图16）。</span><span class="sxs-lookup"><span data-stu-id="93ded-456">Choose **Refresh** (see Figure 16).</span></span> <span data-ttu-id="93ded-457">再次选择所有任务，最上面的任务显示已完成80%。</span><span class="sxs-lookup"><span data-stu-id="93ded-457">All of the tasks are selected again, and the top task shows 80% complete.</span></span> 
    
      <span data-ttu-id="93ded-458">**图16。刷新快速状态更新页面**</span><span class="sxs-lookup"><span data-stu-id="93ded-458">**Figure 16. Refreshing the Quick Status Update page**</span></span>

      <span data-ttu-id="93ded-459">![刷新 QuickStatus 页面](media/pj15_CreateStatusingApp_Testing2Refresh.gif "刷新 QuickStatus 页面")</span><span class="sxs-lookup"><span data-stu-id="93ded-459">![Refreshing the QuickStatus page](media/pj15_CreateStatusingApp_Testing2Refresh.gif "Refreshing the QuickStatus page")</span></span>
  
4. <span data-ttu-id="93ded-460">清除所有复选框，然后选择 "另一任务"。</span><span class="sxs-lookup"><span data-stu-id="93ded-460">Clear all of the check boxes, and then select another task.</span></span> <span data-ttu-id="93ded-461">例如，**从 PWA 中选择 "新建任务**"。</span><span class="sxs-lookup"><span data-stu-id="93ded-461">For example, select **New task from PWA**.</span></span> <span data-ttu-id="93ded-462">将 "**设置完成百分比**" 文本框保留为空，删除选定任务的 "**完成**百分比" 列中的所有文本，然后选择 "**更新**"。</span><span class="sxs-lookup"><span data-stu-id="93ded-462">Leave the **Set percent complete** text box empty, delete all text in the **% complete** column for the selected task, and then choose **Update**.</span></span> <span data-ttu-id="93ded-463">由于两个文本框均为空，因此应用程序显示红色错误消息（见图17）。</span><span class="sxs-lookup"><span data-stu-id="93ded-463">Because both text boxes are empty, the app shows a red error message (see Figure 17).</span></span>
    
      <span data-ttu-id="93ded-464">**图17。测试错误消息**</span><span class="sxs-lookup"><span data-stu-id="93ded-464">**Figure 17. Testing the error message**</span></span>

      <span data-ttu-id="93ded-465">![测试错误消息](media/pj15_CreateStatusingApp_Testing3Error.gif "测试错误消息")</span><span class="sxs-lookup"><span data-stu-id="93ded-465">![Testing the error message](media/pj15_CreateStatusingApp_Testing3Error.gif "Testing the error message")</span></span>
  
5. <span data-ttu-id="93ded-466">将上一个任务更新为已完成的80%，然后选择 "**退出**"。</span><span class="sxs-lookup"><span data-stu-id="93ded-466">Update the previous task to 80% complete, and then choose **Exit**.</span></span> <span data-ttu-id="93ded-467">**ExitToPwa**函数将浏览器窗口的位置更改为 SharePoint 主机应用程序中的任务页面（即 URL 更改为https://ServerName/pwa/Tasks.aspx)）。</span><span class="sxs-lookup"><span data-stu-id="93ded-467">The **exitToPwa** function changes the browser window location to the Tasks page in the SharePoint host application (that is, the URL changes to https://ServerName/pwa/Tasks.aspx).</span></span> <span data-ttu-id="93ded-468">图18显示了**T1**任务和**来自 PWA 任务的新任务**，每个显示的完成百分比为80%。</span><span class="sxs-lookup"><span data-stu-id="93ded-468">Figure 18 shows that the **T1** task and the **New task from PWA** task each show 80% complete.</span></span> 
    
      <span data-ttu-id="93ded-469">**图18。验证 Project Web App 中的任务是否已更新**</span><span class="sxs-lookup"><span data-stu-id="93ded-469">**Figure 18. Verifying the tasks are updated in Project Web App**</span></span>

      <span data-ttu-id="93ded-470">![验证 Project Web App 中的已更新任务](media/pj15_CreateStatusingApp_TasksUpdatedInPWA.gif "验证 Project Web App 中的已更新任务")</span><span class="sxs-lookup"><span data-stu-id="93ded-470">![Verifying the updated tasks in Project Web App](media/pj15_CreateStatusingApp_TasksUpdatedInPWA.gif "Verifying the updated tasks in Project Web App")</span></span>
  
6. <span data-ttu-id="93ded-471">在 Project Professional 2013 中显示已更新的状态之前，必须提交更改以供审批，然后由项目经理批准。</span><span class="sxs-lookup"><span data-stu-id="93ded-471">Before the updated status shows in Project Professional 2013, the changes must be submitted for approval, and then approved by the project manager.</span></span>
    
<span data-ttu-id="93ded-472">测试显示了在**QuickStatus**应用程序中应进行的几个其他更改，以提高可用性。</span><span class="sxs-lookup"><span data-stu-id="93ded-472">Testing reveals several other changes that should be made in the **QuickStatus** app for improved usability.</span></span> <span data-ttu-id="93ded-473">例如：</span><span class="sxs-lookup"><span data-stu-id="93ded-473">For example:</span></span>

- <span data-ttu-id="93ded-474">应对文本框的值进行其他错误检查和验证。</span><span class="sxs-lookup"><span data-stu-id="93ded-474">There should be additional error checks and validation of text box values.</span></span> <span data-ttu-id="93ded-475">目前，用户可以输入一个非数值或负值的完成百分比，这将导致 unfriendly 错误消息。</span><span class="sxs-lookup"><span data-stu-id="93ded-475">Currently, a user can enter a non-numeric value or a negative value for percent complete, which results in an unfriendly error message.</span></span> <span data-ttu-id="93ded-476">例如，如果值为负值，则错误消息会在**更新工作分配时出错： PJClientCallableException： StatusingSetDataValueInvalid**。</span><span class="sxs-lookup"><span data-stu-id="93ded-476">For example, with a negative value, the error message is **Error updating assignments: PJClientCallableException: StatusingSetDataValueInvalid**.</span></span>
    
- <span data-ttu-id="93ded-477">空白文本框的错误消息可以列出项目和任务，以及行号。</span><span class="sxs-lookup"><span data-stu-id="93ded-477">The error message for blank text boxes could list the project and task, in addition to the row number.</span></span>
    
- <span data-ttu-id="93ded-478">成功的消息可能包括更新的任务的列表;如果**updateAssignments**函数成功，它可以执行自动页面刷新，并以不同的颜色和粗体字显示更新的任务或百分比。</span><span class="sxs-lookup"><span data-stu-id="93ded-478">The success message could include a list of the tasks updated; or if the **updateAssignments** function is successful, it could perform an automatic page refresh and show updated tasks or percentages in a different color and bold font.</span></span> 
    
- <span data-ttu-id="93ded-479">若要避免一个非常大的表格，工作分配的表应限制为少于100% 的任务已完成。</span><span class="sxs-lookup"><span data-stu-id="93ded-479">To avoid a very large table, the table of assignments should be limited to tasks that are less than 100% complete.</span></span> <span data-ttu-id="93ded-480">或者，添加一个选项以显示所有任务。</span><span class="sxs-lookup"><span data-stu-id="93ded-480">Or, add an option to show all tasks.</span></span> <span data-ttu-id="93ded-481">此外，还可以使用基于 jQuery 的网格（而不是表）来解决此问题，您可以在其中轻松实现筛选和网格分页。</span><span class="sxs-lookup"><span data-stu-id="93ded-481">This problem could also be solved by using a jQuery-based grid instead of a table, where you can easily implement filtering and grid paging.</span></span>
    
- <span data-ttu-id="93ded-482">由于**QuickStatus**应用不会提交状态，功能区的 "**任务**" 选项卡上的 "**快速状态**" 图标将更逻辑地成为 "**任务**" 组中的第一个图标，而不是 "**提交**" 组中的最后一个图标。</span><span class="sxs-lookup"><span data-stu-id="93ded-482">Because the **QuickStatus** app does not submit status, the **Quick Status** icon on the **TASKS** tab of the ribbon would more logically be the first icon in the **Tasks** group, rather than the last icon in the **Submit** group.</span></span> 
    
- <span data-ttu-id="93ded-483">由于**onGetAssignmentsSuccess**函数初始化**btnSubmitUpdate**按钮文本，但在 HTML 中初始化其他按钮文本值，因此在运行**getAssignments**函数时，该页面将保持部分初始化状态。</span><span class="sxs-lookup"><span data-stu-id="93ded-483">Because the **onGetAssignmentsSuccess** function initializes the **btnSubmitUpdate** button text, but the other button text values are initialized in HTML, the page is left in a partially initialized state while the **getAssignments** function runs.</span></span> <span data-ttu-id="93ded-484">如果文本值全部以 HTML 格式初始化，则页面上的按钮将显示为更一致。</span><span class="sxs-lookup"><span data-stu-id="93ded-484">Buttons on the page would appear more consistent if the text values were all initialized in HTML.</span></span> 
    
<span data-ttu-id="93ded-485">最重要的是， **QuickStatus**应用程序使用的方法，在其中更改工作分配的完成百分比应在生产应用中进行修订。</span><span class="sxs-lookup"><span data-stu-id="93ded-485">Most importantly, the approach that the **QuickStatus** app uses, where it changes percent complete for assignments, should be revised in a production app.</span></span> <span data-ttu-id="93ded-486">有关详细信息，请参阅 "[后续步骤](#pj15_StatusingApp_NextSteps)" 部分。</span><span class="sxs-lookup"><span data-stu-id="93ded-486">For more information, see the [Next steps](#pj15_StatusingApp_NextSteps) section.</span></span> 

<span data-ttu-id="93ded-487"><a name="pj15_StatusingApp_Example"> </a></span><span class="sxs-lookup"><span data-stu-id="93ded-487"><a name="pj15_StatusingApp_Example"> </a></span></span>

## <a name="example-code-for-the-quickstatus-app"></a><span data-ttu-id="93ded-488">QuickStatus 应用程序示例代码</span><span class="sxs-lookup"><span data-stu-id="93ded-488">Example code for the QuickStatus app</span></span>

### <a name="defaultaspx-file"></a><span data-ttu-id="93ded-489">默认 .aspx 文件</span><span class="sxs-lookup"><span data-stu-id="93ded-489">Default.aspx file</span></span>

<span data-ttu-id="93ded-490">以下代码位于**QuickStatus**项目的`Pages\Default.aspx`文件中：</span><span class="sxs-lookup"><span data-stu-id="93ded-490">The following code is in the  `Pages\Default.aspx` file of the **QuickStatus** project:</span></span> 
...
### <a name="appjs-file"></a><span data-ttu-id="93ded-491">应用 .js 文件</span><span class="sxs-lookup"><span data-stu-id="93ded-491">App.js file</span></span>

<span data-ttu-id="93ded-492">以下代码位于**QuickStatus**项目的`Scripts\App.js`文件中：</span><span class="sxs-lookup"><span data-stu-id="93ded-492">The following code is in the  `Scripts\App.js` file of the **QuickStatus** project:</span></span> 
...
### <a name="appcss-file"></a><span data-ttu-id="93ded-493">App.css 文件</span><span class="sxs-lookup"><span data-stu-id="93ded-493">App.css file</span></span>

<span data-ttu-id="93ded-494">以下 CSS 代码位于**QuickStatus**项目的`Content\App.css`文件中：</span><span class="sxs-lookup"><span data-stu-id="93ded-494">The following CSS code is in the  `Content\App.css` file of the **QuickStatus** project:</span></span> 
...
### <a name="elementsxml-file-for-the-ribbon"></a><span data-ttu-id="93ded-495">功能区的元素 .xml 文件</span><span class="sxs-lookup"><span data-stu-id="93ded-495">Elements.xml file for the ribbon</span></span>

<span data-ttu-id="93ded-496">以下 XML 定义适用于功能区上 "**任务**" 选项卡上的 "已添加" 按钮`RibbonQuickStatusAction\Elements.xml` ，位于**QuickStatus**项目的文件中：</span><span class="sxs-lookup"><span data-stu-id="93ded-496">The following XML definition, for the added button on the **TASKS** tab on the ribbon, is in the  `RibbonQuickStatusAction\Elements.xml` file of the **QuickStatus** project:</span></span> 
...
### <a name="appmanifestxml-file"></a><span data-ttu-id="93ded-497">AppManifest.xml 文件</span><span class="sxs-lookup"><span data-stu-id="93ded-497">AppManifest.xml file</span></span>

<span data-ttu-id="93ded-498">以下是**QuickStatus**项目的应用程序清单的 XML，其中包括在多个项目中更新应用程序用户的分配状态所需的两个权限请求范围：</span><span class="sxs-lookup"><span data-stu-id="93ded-498">Following is the XML for the app manifest of the **QuickStatus** project, which includes the two permission request scopes that are necessary for updating the app user's assignment status in multiple projects:</span></span> 
...
### <a name="appiconpng-file"></a><span data-ttu-id="93ded-499">AppIcon .png 文件</span><span class="sxs-lookup"><span data-stu-id="93ded-499">AppIcon.png file</span></span>

<span data-ttu-id="93ded-500">**QuickStatus**应用程序的完整 Visual Studio 解决方案包含自定义的 AppIcon .png 文件。</span><span class="sxs-lookup"><span data-stu-id="93ded-500">The complete Visual Studio solution for the **QuickStatus** app includes a custom AppIcon.png file.</span></span> <span data-ttu-id="93ded-501">解决方案将包含在 Project 2013 SDK 下载中。</span><span class="sxs-lookup"><span data-stu-id="93ded-501">The solution will be included in the Project 2013 SDK download.</span></span> 

<span data-ttu-id="93ded-502"><a name="pj15_StatusingApp_NextSteps"> </a></span><span class="sxs-lookup"><span data-stu-id="93ded-502"><a name="pj15_StatusingApp_NextSteps"> </a></span></span>

## <a name="next-steps"></a><span data-ttu-id="93ded-503">后续步骤</span><span class="sxs-lookup"><span data-stu-id="93ded-503">Next steps</span></span>

<span data-ttu-id="93ded-504">**QuickStatus**应用程序是如何编写可在 project Server 2013 和 project Online 上安装的应用程序的相对简单的示例。</span><span class="sxs-lookup"><span data-stu-id="93ded-504">The **QuickStatus** app is a relatively simple example of how to write apps that can be installed on Project Server 2013 and Project Online.</span></span> <span data-ttu-id="93ded-505">[测试 QuickStatus 应用程序](#pj15_StatusingApp_Testing)部分列出了可进行的多种改进以提高可用性。</span><span class="sxs-lookup"><span data-stu-id="93ded-505">The [Testing the QuickStatus app](#pj15_StatusingApp_Testing) section lists several improvements that can be made for better usability.</span></span> <span data-ttu-id="93ded-506">**QuickStatus**应用使用 JavaScript 函数更新 Project Web app 的工作分配状态。</span><span class="sxs-lookup"><span data-stu-id="93ded-506">The **QuickStatus** app uses JavaScript functions to update assignment status for Project Web App.</span></span> <span data-ttu-id="93ded-507">但是，更改分配的完成百分比不是建议的项目管理实践。</span><span class="sxs-lookup"><span data-stu-id="93ded-507">But, changing the assignment percent complete is not a recommended project management practice.</span></span> <span data-ttu-id="93ded-508">另一种方法是更新所分配任务的实际开始日期和剩余持续时间。</span><span class="sxs-lookup"><span data-stu-id="93ded-508">Another approach would be to update the actual start date and remaining duration of assigned tasks.</span></span> <span data-ttu-id="93ded-509">有关这些问题的讨论，请参阅 MPUG 新闻稿中[更好的更新](https://www.mpug.com/articles/update-better)。</span><span class="sxs-lookup"><span data-stu-id="93ded-509">For a discussion of the issues, see [Update Better](https://www.mpug.com/articles/update-better) in the MPUG newsletter.</span></span> 

<span data-ttu-id="93ded-510"><a name="pj15_StatusingApp_AdditionalResources"> </a></span><span class="sxs-lookup"><span data-stu-id="93ded-510"><a name="pj15_StatusingApp_AdditionalResources"> </a></span></span>

## <a name="see-also"></a><span data-ttu-id="93ded-511">另请参阅</span><span class="sxs-lookup"><span data-stu-id="93ded-511">See also</span></span>

- [<span data-ttu-id="93ded-512">Project Server 编程任务</span><span class="sxs-lookup"><span data-stu-id="93ded-512">Project Server programming tasks</span></span>](project-programming-tasks.md)
- [<span data-ttu-id="93ded-513">SharePoint 外接程序</span><span class="sxs-lookup"><span data-stu-id="93ded-513">SharePoint Add-ins</span></span>](https://msdn.microsoft.com/library/jj163230.aspx)
- [<span data-ttu-id="93ded-514">管理 Project Web App 中的任务更新</span><span class="sxs-lookup"><span data-stu-id="93ded-514">Managing task updates in Project Web App</span></span>](https://technet.microsoft.com/library/hh767481%28v=office.14%29.aspx)
- [<span data-ttu-id="93ded-515">创建自定义操作以部署 SharePoint 外接程序</span><span class="sxs-lookup"><span data-stu-id="93ded-515">Create custom actions to deploy with SharePoint Add-ins</span></span>](https://msdn.microsoft.com/library/jj163954.aspx)
