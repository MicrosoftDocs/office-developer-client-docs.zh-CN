...
如果某个动作标记没有匹配动作，则该动作标记按钮仍会按照定义方式显示和消失，但其菜单为空白。
  
您只能在 shapesheet 窗口中插入**Action 标记**部分 (右键单击 shapesheet 窗口, 单击 "**插入内容**", 然后选中 "**操作标记**" 复选框)。 若要将操作标记添加到现有的**操作标记**部分, 请右键单击某一行, 然后单击快捷菜单上的 "**插入行**"。 
