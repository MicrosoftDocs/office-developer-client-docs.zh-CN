...
包含  *x*  坐标和  *y*  坐标、水平和垂直方向，以及形状上的单个连接点的类型。 连接点的坐标以形状的原点为基准进行测量。 形状上的每个连接点都包含一个 Connection Points 行。 
  
如果命名了多个 Connection Points 行，则它们的名称将以 Connections. *名称*  。 Connection Points 行包含以下单元格。 有关详细信息，请参阅特定的单元格主题。 
  
|**Cell**|**说明**|
|:-----|:-----|
|[X](x-cell-connection-points-section.md) <br/> |本地坐标中连接点的  *x*  坐标。  <br/> |
|[Y](y-cell-connection-points-section.md) <br/> |本地坐标中连接点的  *y*  坐标。  <br/> |
|[DirX/A](dirxa-cell-connection-points-section.md) <br/> |匹配连接点所需的对齐矢量的  *x*  分量。 它还用于确定动态连接线所附引线的方向。 此单元格采用浮点值。  <br/> |
|[DirY/B](diryb-cell-connection-points-section.md) <br/> |匹配连接点所需的对齐矢量的  *y*  分量。 它还用于确定动态连接线所附引线的方向。 此单元格采用浮点值。  <br/> |
...
Connections. *name*  行标有 DirX/A、DirY/B 和 Type/C，因为这些行可以是扩展行，也可以不是扩展行。 
...
扩展行有 A、B、C 和 D 单元格，其行为类似“向内”类型的无方向的非扩展行。通常不使用扩展行，但在将数据与 A、B、C 和 D 单元格中的连接点相关联时可能要使用它们。它们的行类型为 **visTagCnnctPtABCD** 或 **visTagCnnctNamedABCD**。如果 D 单元格中存在公式，便可识别这是扩展行。 
 ...
 可以根据需要添加任意多的 Connections.  *命名*  行（如需要）为行指定有意义的名称，并设置单元格值。 若要向现有的 Connection Points 内容添加连接点，请右击某一行，然后单击快捷菜单上的 **“插入行”**。 
  
可以按行名称引用 Connection Points 行单元格，这些行名称以红色文本显示在 ShapeSheet 窗口中。 若要更改行名称，请单击它，然后键入一个名称，如自定义  *，例如*  ，创建行名称 Connections.Custom。 然后，如果要使用行号，可以使用 Connections.Custom.X 或 Connections.X1 引用 X 单元格。 
  
您输入的行名称在该内容中必须是唯一的。 为"Connection Points"内容中的一行创建名称时，Microsoft Office Visio默认名称为"n"来命名该内容Connections.Row_ *行*。 
