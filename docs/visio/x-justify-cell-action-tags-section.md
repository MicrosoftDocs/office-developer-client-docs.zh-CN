...
动作  *标记*  按钮相对于 X 单元格和 Y 单元格定义的点的 x 偏移量。 
...
X Justify 和 Y Justify 单元格确定动作标记按钮相对于 X 单元格和 Y 单元格中定义的点的放置位置。 
  
若要从另一个公式或从使用 **CellsU** 属性的某个程序按名称获取对 X Justify 单元格的引用，请使用： 
  
|||
|:-----|:-----|
| 单元格名称：  <br/> | SmartTags。  *name*  .XJustify 其中 SmartTags. *name*  是动作标记行的名称  <br/> |
