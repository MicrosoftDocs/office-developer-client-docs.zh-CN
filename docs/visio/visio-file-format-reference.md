---
title: Visio 文件格式参考
manager: soliver
ms.date: 11/16/2014
ms.audience: Developer
ms.topic: reference
localization_priority: Normal
ms.assetid: 38aedff8-e7b5-0758-c573-0ffa6a244c9c
description: 本参考包含一个概念性概述主题和参考主题 (包括示例), 以指导您使用 Visio 文件格式开发解决方案。
ms.openlocfilehash: 28e7541515720c5516ce45cd48274becedd63cf3
ms.sourcegitcommit: 8fe462c32b91c87911942c188f3445e85a54137c
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 04/23/2019
ms.locfileid: "32335600"
---
# <a name="visio-file-format-reference"></a>Visio 文件格式参考

本参考包含一个概念性概述主题和参考主题 (包括示例), 以指导您使用 Visio 文件格式 (\*.vsdx) 开发解决方案。
  
我们欢迎有关此内容的评论或疑问。 可以发送反馈到[docthis@microsoft.com](mailto:docthis@microsoft.com)。
  
 **初始发布**:10 月 2012 |**更新**时间: 2013 年3月
  
本文档包含以下信息：
  
- [Visio 文件格式 (.vsdx) 简介](introduction-to-the-visio-file-formatvsdx.md)
    
- [以编程方式处理 Visio 文件格式](how-to-manipulate-the-visio-file-format-programmatically.md)
    
- [架构映射](schema-mapvisio-xml.md): 显示 Visio 文件格式的 XML 架构。
    
- [元素](elementsvisio-xml.md): 提供有关可在 Visio 文件格式中显示的 XML 元素的信息。
    
- [类型](typesvisio-xml.md): 提供有关可在 Visio 文件格式中显示的 XML 类型的信息。
    
## <a name="see-also"></a>另请参阅

- [Visio 开发人员参考](https://msdn.microsoft.com/office/aa905478.aspx) 
- [Microsoft 产品中的辅助功能](https://www.microsoft.com/enable/products/default.aspx)
- [文档约定](https://msdn.microsoft.com/office/aa905365.aspx) 
- [Visio SDK 版权通知](visio-sdk-copyright-notice.md)

    

