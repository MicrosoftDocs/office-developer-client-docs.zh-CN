...
您还可以设置此值**填充**对话框中使用滑块控件 （在**主页**选项卡，**形状**组中，单击**填充**，然后单击**填充选项**）。 此值控制前景色和背景填充的透明度的值。 若要独立设置这些值，必须在 ShapeSheet 窗口中输入它们。
  
要从另一个公式或使用**CellsU**属性从某个程序按名称获取对 FillForegndTrans 单元格的引用，请使用： 
