...
## <a name="quick-filing-dialog-box"></a>快速归档对话框
...
![没有自定义的快速归档对话框]没有(media/ON15Con_quick_filing_dialog.jpg "自定义的快速归档对话框")
...
|||
|:-----|:-----|
|**描述** <br/> |设置哪些最近的结果列表将显示在快速归档对话框中，并指示是否在列表中包括一些特殊归档位置。用户可以选择 [RecentResultType](enumerations-onenote-developer-reference.md#odc_RecentResultType)枚举中的最新的结果列表。用户还可以选择以下选项添加到列表: 当前节、 当前页或未归档笔记。如果 **RecentResultType.rrtNone** 处于选中状态，将不显示任何新的结果列表。 <br/> |
...
|**参数** <br/> | _recentResults_ &ndash; 一个 **类型为 RecentResultType** 的对象，指示应显示哪个最近的结果列表（如果有）。 如果 **rrtNone** 处于选中状态，没有最新的结果列表将显示在对话框中。<br/><br/>  _fShowCurrentSection_ &ndash; 一个布尔值，指示当前节是否应该包括在当前结果列表中。<br/><br/>  _fShowCurrentPage_ &ndash; 一个 Boolean 值，指示当前页面是否应该包含在最近的结果列表中。<br/><br/>  _fShowUnfiledNotes_ &ndash; 一个 Boolean 值，指示"未筛选的注释"部分是否应该包括在最近的结果列表中。  <br/> |
...
|**参数** <br/> | _bstrText_ &ndash; 指定按钮上显示的文本的字符串。 要自定义的默认 **确定** 按钮，将作为 **bstrText** 传递中 null 值。  <br/><br/>_allowedElements_ &ndash;**一个 HierarchyElement，** 指示OneNote按钮允许用户选择哪些非只读层次结构元素。 选择多个项目，用户应通过 **OR** 运算符中的所有 **HierarchyElement** 类型允许作为 **HierarchyElement** uint 等效值。<br/><br/>  _allowedReadOnlyElements_ &ndash;**一个 HierarchyElement，OneNote** 按钮允许用户选择哪些只读层次结构元素。 选择多个项目，用户应通过在 **OR** 运算符 **HierarchyElement** 类型允许作为 **HierarchyElement** 的所有 **uint** 等效值。<br/><br/>  _fDefault_ &ndash; 一个布尔值，指定此按钮是否应该是默认按钮。 如果多个按钮设置为默认值，最后一个指定的按钮将成为默认按钮。  <br/> |
...
|||
|:-----|:-----|
|**描述** <br/> |在新线程中的快速归档对话框中显示。计 **IQuickFilingDialogCallback** 接口，关闭该对话框后，将调用其 **OnDialogClosed** 方法的引用。 <br/> |
|**语法** <br/> | `HRESULT Run (`<br/>`[in]IQuickFilingDialogCallback piCallback);` <br/> |
|**参数** <br/> | _piCallback_ &ndash; 对 **IQuickFilingDialogCallback** 接口的引用，该接口将在对话框关闭后实例化。  <br/> |
...
|**参数** <br/> | _dialog_ &ndash;调用 **OnDialogClose** 方法的 **IQuickFilingDialog** 对象。  <br/> |
