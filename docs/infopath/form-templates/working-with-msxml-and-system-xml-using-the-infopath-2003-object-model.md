...
若要使用的 InfoPath 2003 对象模型的托管的代码项目中使用**System.Xml**命名空间的成员，必须在**添加引用**对话框的 Visual Studio 2012 中的 **.NET**选项卡添加对**System.Xml** 。 
  
 **备注**
  
- 若要查看有关 MSXML 的参考信息，请参阅 MSXML SDK。
...
- 如果您更新的表单模板使用由[汇总了 Microsoft.Office.InfoPath](https://msdn.microsoft.com/library/Microsoft.Office.InfoPath.aspx)命名空间的成员提供的对象模型的代码，以本机模式使用**System.Xml** 。 但是，当这样，您必须手动转换所有代码以使用新的对象模型。 要转换表单模板的**表单选项**对话框的**编程**类别中使用新的对象模型中，单击**升级 OM**。
...
下面的示例要求在表单代码模块的声明部分对 **System.Xml** 使用 **using** 或 **Imports** 指令。 此外，由于**XmlDocument**类的**Load**方法要求**System.Security.Permissions.FileIOPermission**，您必须配置表单模板的安全级别为**完全信任**使用**安全和信任****表单选项**对话框的类别。 
