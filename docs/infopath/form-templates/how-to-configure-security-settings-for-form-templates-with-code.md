...
> [!NOTE]
> <span data-ttu-id="811cb-p103">通过在 **".NET Configuration 2.0"** 管理单元中右键单击安全代码组，单击 **"属性"**，再单击 **"权限集"** 选项卡，您可以随时更改该组的权限集。</span><span class="sxs-lookup"><span data-stu-id="811cb-p103">You can change the permission set for a security code group at any time by right-clicking the group in the . **NET Configuration 2.0** snap-in, clicking **Properties**, and then clicking the **Permission Set** tab.</span></span> 
