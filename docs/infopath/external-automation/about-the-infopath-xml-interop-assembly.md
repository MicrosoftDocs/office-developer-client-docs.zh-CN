...
InfoPath 安装程序中的 **.NET 可编程性支持**选项安装三个互操作程序集。 互操作程序集是充当托管代码与非托管代码之间桥梁的 .NET 程序集，它将 COM 对象成员映射到等同的 .NET 托管成员。 这些程序集，Microsoft.Office.Interop.InfoPath.Xml.dll，之一提供用于处理由 COM 服务器公开 Microsoft XML Core Services (MSXML) 的成员的[Microsoft.Office.Interop.InfoPath.Xml](https://docs.microsoft.com/dotnet/api/microsoft.office.interop.infopath.xml?view=infopath-external)命名空间的成员从外部应用程序的自动执行 InfoPath 托管的代码使用。 
