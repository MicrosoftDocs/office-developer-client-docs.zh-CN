---
title: 存储区
TOCTitle: Stores
ms:assetid: ce5d8e58-7753-485c-b400-4f00f6c6935b
ms:mtpsurl: https://msdn.microsoft.com/library/Ff184642(v=office.15)
ms:contentKeyID: 55119892
ms.date: 07/24/2014
mtps_version: v=office.15
localization_priority: Normal
ms.openlocfilehash: 2e0e8699dc64f00cca365ae2bd9ae84edab96c52
ms.sourcegitcommit: 8fe462c32b91c87911942c188f3445e85a54137c
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 04/23/2019
ms.locfileid: "32357678"
---
# <a name="stores"></a>存储

本节收录了与存储相关的示例任务。 存储表示本地计算机上的文件，或存储电子邮件和其他项的网络驱动器。

## <a name="in-this-section"></a>本节内容

|主题|说明|
|:----|:----------|
|[获取配置文件中的存储的相关信息](how-to-get-information-about-stores-in-a-profile.md)  |介绍了如何获取并枚举配置文件中的存储。|
|[添加或删除存储](how-to-add-or-remove-a-store.md)  |介绍了如何在给定配置文件中添加或删除存储。|

## <a name="see-also"></a>另请参阅

- [帐户](accounts.md)
- [文件夹](folders.md)
- [我如何...（Outlook 2013 PIA 参考）](how-do-i-outlook-2013-pia-reference.md)

