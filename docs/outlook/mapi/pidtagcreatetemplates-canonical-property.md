...
若要了解可以在容器内创建哪些模板对象，请对此属性调用 [IMAPIProp：：OpenProperty](imapiprop-openproperty.md) 方法。 生成的对象是一对一表，它提供可在容器内创建的所有模板的条目标识符。 
  
若要创建模板对象，请对 PR_ENTRYID ([PidTagEntryId](pidtagentryid-canonical-property.md)) 一次表中的列值调用容器对象的 **CreateEntry** 方法。 
