...
包含邮件第一个版本的发件人的条目标识符，即转发或答复前的邮件。
...
此属性是邮件的原始发件人的地址属性之一。 首次提交邮件时，客户端应用程序应该将此属性设置为 PR_SENDER_ENTRYID ([PidTagSenderEntryId](pidtagsenderentryid-canonical-property.md)) 属性的值。  转发或答复邮件时从不更改。
