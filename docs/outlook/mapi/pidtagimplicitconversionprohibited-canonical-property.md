...
如果禁止邮件传输代理 (MTA) 进行隐式邮件文本转换，则包含 TRUE。
...
如果此属性为 TRUE，则邮件系统不得对邮件执行任何内容转换，除非使用 **PR_EXPLICIT_CONVERSION** ([PidTagExplicitConversion](pidtagexplicitconversion-canonical-property.md)) 属性按收件人显式请求。
