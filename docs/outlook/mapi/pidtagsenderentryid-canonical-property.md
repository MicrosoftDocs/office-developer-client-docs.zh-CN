...
此属性是邮件发件人的地址属性之一。 它必须由传出传输提供程序设置, 该提供程序决不会传播任何以前存在的值。
  
如果没有任何传输提供程序提供任何发件人地址属性, MAPI 后台处理程序将尝试通过调用条目标识符的[IMAPISession:: QueryIdentity](imapisession-queryidentity.md)方法来填充这些属性。 如果未提供条目标识符, 则 MAPI 后台处理程序将对应于此属性中的字符串 "Unknown" 的标识符。 
