...
此属性用于使用 [IMAPISession：：OpenMsgStore 方法打开邮件](imapisession-openmsgstore.md) 存储。 它还用于打开邮件存储拥有的任何对象。 
  
对于邮件存储，此属性与[PidTagEntryId](pidtagentryid-canonical-property.md)属性PR_ENTRYID (存储) 属性。 客户端应用程序可以使用 [IMAPISession：：CompareEntryIDs](imapisession-compareentryids.md) 方法比较这两个属性。 
