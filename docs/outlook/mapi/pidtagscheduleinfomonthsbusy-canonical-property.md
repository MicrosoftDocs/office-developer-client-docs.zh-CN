...
包含忙/闲邮件中存在类型为 "忙" 的忙/闲数据的月份。
  
|||
|:-----|:-----|
|相关属性：  <br/> |PR_SCHDINFO_MONTHS_BUSY  <br/> |
|标识符:  <br/> |0x6853  <br/> |
...
此属性的格式、计算和约束与**PR_SCHDINFO_MONTHS_TENTATIVE** ([PidTagScheduleInfoMonthsTentative](pidtagscheduleinfomonthstentative-canonical-property.md)) 的格式、计算和约束相同, 但引用在关联的日历对象上标记为 "忙碌" 的约会。
