...
|LONG ID (的一) ：  <br/> |0x00002328  <br/> |
|数据类型：  <br/> |PT_MV_UNICODE  <br/> |
|区域：  <br/> |常见  <br/> |
   
## <a name="remarks"></a>备注

若要生成关键字标头字段，客户端必须将此属性的值设置为所需的值。 此属性具有多个字符串;每个类别应映射到一个关键字。
  
多用途 Internet 邮件扩展 (MIME) 编写器应该将此属性的每个子值复制到 Keywords 头字段中的单独关键字，其中用逗号 (U+002C) 和空格 (U+0020) 分隔每个关键字。 MIME 编写器可能会删除此属性，而不是将其复制到关键字标头字段，以避免不同组织中不同类别集间发生冲突。
