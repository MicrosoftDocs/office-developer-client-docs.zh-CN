...
> [in]指向用于访问打开条目的接口的接口标识符 (IID) 的指针。 传递 NULL 将返回标准接口的对象。 对于邮件用户，标准接口是[IMailUser: IMAPIProp](imailuserimapiprop.md)。 对于通讯组列表它是[IDistList: IMAPIContainer](idistlistimapicontainer.md)，以及是的容器[IABContainer: IMAPIContainer](iabcontainerimapicontainer.md)。 呼叫者可以设置_lpInterface_为相应的标准接口或继承层次结构中的接口。 
