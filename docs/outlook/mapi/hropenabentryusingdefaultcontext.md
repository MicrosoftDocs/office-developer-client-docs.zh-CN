...
执行与 [HrOpenABEntryWithExchangeContext](hropenabentrywithexchangecontext.md) 相同的功能，只不过它使用旧的 **emsmdbUID** 作为  _pEmsmdbUID_ 参数。 除非无法获取正确的 **emsmdbUID** 调用 [HrOpenABEntryWithExchangeContext](hropenabentrywithexchangecontext.md)，否则请不要使用此函数。
...
> 仅使用脱机通讯簿来执行名称解析。 例如，您可以使用此标志允许客户端应用程序在缓存 exchange 模式下打开全局地址列表 (GAL) ，并访问缓存中的该通讯簿中的条目，而无需在客户端和服务器之间创建流量。 只有通讯簿提供商才支持Exchange标志。
...
> 仅使用 GAL 执行名称解析。 只有通讯簿提供商才支持Exchange标志。
...
> 不使用脱机通讯簿执行名称解析。 只有通讯簿提供商才支持Exchange标志。
