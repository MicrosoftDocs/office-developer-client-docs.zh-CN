...
包含忙/闲邮件中出现忙/闲类型的忙/闲数据或外出 (OOF) 消息的月份列表。 
...
此属性中不包含忙/闲类型暂定事件。 此属性的语法/格式和约束与 **PR_SCHDINFO_MONTHS_TENTATIVE** ([PidTagScheduleInfoMonthsTentative](pidtagscheduleinfomonthstentative-canonical-property.md)) 但指在关联的日历对象上标记为 OOF 或 Busy 的约会。 
