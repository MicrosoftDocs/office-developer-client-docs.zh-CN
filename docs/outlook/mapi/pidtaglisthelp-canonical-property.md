...
包含多用途 Internet 邮件扩展 (MIME) 邮件的列表-帮助头字段的值。
...
若要生成 List Help 标头字段, 客户端必须将**PR_LIST_HELP**或关联属性的值设置为所需的值。 MIME 编写器必须将此值复制到列表帮助头字段。 
  
若要设置这些列表服务器相关属性的值, MIME 客户端必须按下表中指定的方式写入头字段:
  
|**Property**|**首选标头字段名称**|**备用标头字段名称**|
|:-----|:-----|:-----|
|**PR_LIST_HELP** <br/> |List-Help  <br/> |X-列表-帮助  <br/> |
