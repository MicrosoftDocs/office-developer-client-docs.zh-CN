...
这些属性是所有邮件用户通用的基本地址属性的示例。 它指定 MAPI 用于处理给定邮件的邮件系统 MAPI。
  
此属性还确定**PR_EMAIL_ADRESS** ([PidTagEmailAddress](pidtagemailaddress-canonical-property.md)) 中的地址字符串的格式。 它提供的字符串只能包含从 A 到 Z 的大写字母字符以及从0到9的数字。
