 ...
 **OpenAddressBook**返回 MAPI_W_ERRORS_RETURNED，如果它无法加载一个或多个配置文件中的地址簿提供程序。 此值是一条警告，不是一个错误值;就像 S_OK 处理它。 **OpenAddressBook**始终_lppAdrBook_参数，无论多少的通讯簿提供程序无法加载中返回一个有效的指针。 因此，您必须始终在通讯簿[IUnknown::Release](https://msdn.microsoft.com/library/ms682317%28v=VS.85%29.aspx)方法某个时间点之前调用注销。 
