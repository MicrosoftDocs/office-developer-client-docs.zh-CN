...
|LONG ID (的一) ：  <br/> |0x00000023  <br/> |
|数据类型：  <br/> |PT_BINARY  <br/> |
|区域：  <br/> |会议  <br/> |
   
## <a name="remarks"></a>备注

此属性的格式与[PidLidGlobalObjectId](pidlidglobalobjectid-canonical-property.md) LID_GLOBAL_OBJID (的格式) 。  此属性的值必须等于 **LID_GLOBAL_OBJID，** 但 YH、YL、M 和 D 字段必须为零。 引用定期系列实例的所有对象 (包括孤立实例) 以及定期系列本身）的此属性具有相同的值。
