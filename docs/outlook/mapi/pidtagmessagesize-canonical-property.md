...
建议邮件对象公开此属性。 邮件大小指示将邮件从一个邮件存储移到另一个时传输的近似字节数。 是 message 对象上所有属性的大小的总和, 通常比邮件文本大得多。 
  
大多数邮件存储提供程序会为其处理的邮件计算此属性。 但是, 某些邮件存储提供程序不支持此属性。 在任何情况下, 在调用[IMAPIProp:: SaveChanges](imapiprop-savechanges.md)或[IMessage:: SubmitMessage](imessage-submitmessage.md)方法之前, 此属性不可用。 
