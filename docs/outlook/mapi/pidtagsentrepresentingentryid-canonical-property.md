...
如果发送客户端已设置传出传输提供程序, 则该提供程序必须始终将此属性保持不变。 如果它未设置, 则传输提供程序应将其设置为邮件的出站副本上的**PR_SENDER_ENTRYID** ([PidTagSenderEntryId](pidtagsenderentryid-canonical-property.md)), 并将其保留在本地副本上。
