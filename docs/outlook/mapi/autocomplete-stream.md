...
读取此流时，如果主要版本不同于 12，则不应读取或写入此流。 自动完成流的当前次要版本为 0，其中额外信息字节计数设置为 0。 如果次要版本不同于 0，则读取流时需要读取的额外信息中会包含一些信息，在编写流时需要保留这些信息。 编写流时还需要保留次要版本。 如果未保留这两者，则撰写额外信息的 Outlook 实例将丢失数据。 
  
> [!NOTE]
> 应用程序不得向"额外信息"字段添加自定义数据或更改次要版本，因为此功能是支持 Outlook 格式扩展，而不是任意第三方扩展。 
...
|保留数据  <br/> |4   <br/> |
|属性值联合  <br/> ||
|数值数据  <br/> |0 或变量 (取决于属性标记)   <br/> |
   
## <a name="interpreting-the-property-value"></a>解释属性值

属性值联合值和值数据根据属性块前 4 个字节中的属性标记进行解释。 此属性标记的格式与 MAPI 属性标记的格式相同。 属性标记的位 0 到 15 是属性的类型。 位 16 到 31 是属性的标识符。 属性类型确定属性的其余部分的读取方法。
  
## <a name="static-value"></a>静态值

某些属性没有值数据，并且只有联合数据。 以下属性类型 (属性标记) 应按如下方式解释 8 字节 Property Union 数据：
  
|**属性类型**|**Property Union Interpretation**|
|:-----|:-----|
...
其他属性在值数据块中包含 Property Tag、保留数据和属性值联合的前 16 个字节之后的数据。 与静态值不同，存储在 8 字节属性值联合中的数据与读取无关。 在编写时，请确保用某些内容填充这 8 个字节。 但是，8 字节的内容并不重要。 在动态值中，属性标记的类型确定如何解释值数据。
...
|:-----|:-----|
|字节数 n  <br/> |4   <br/> |
|要解释为 ANSI 字符串字符串的 (包括 NULL 终止符)   <br/> |n  <br/> |
...
|:-----|:-----|
|二进制数组 X 的数量  <br/> |4   <br/> |
|包含 X 二进制数组的字节数。 每个数组应完全像字节PT_BINARY一样解释。  <br/> |变量  <br/> |
...
|包含 X ANSI 字符串的字节数。 每个字符串应完全像字节PT_STRING8一样解释。  <br/> |变量  <br/> |
...
|包含 X UNICODE 字符串的字节数。 每个字符串应完全像字节PT_UNICODE一样解释。  <br/> |变量  <br/> |
   
## <a name="significant-properties"></a>重要属性

如本主题前面所述，表示属性的二进制块具有与通讯簿收件人的属性对应的属性标记。 对于此处未列出的属性，可在以下网站查找属性说明 https://msdn.microsoft.com/library/cc433490(EXCHG.80).aspx 。
  
|**属性名**|**属性标记**|**说明 (有关详细信息，请参阅 MSDN)**|
|:-----|:-----|:-----|
|PR_NICK_NAME_W (传输给收件人，特定于仅自动完成流)   <br/> |0x6001001f  <br/> |此属性必须在每个收件人行中第一个。 它在功能上充当收件人行的键标识符。  <br/> |
...
|PR_DROPDOWN_DISPLAY_NAME_W (传输给收件人，特定于仅自动完成流)   <br/> |0X6003001f  <br/> |自动完成列表中显示的显示字符串。  <br/> |
|PR_NICK_NAME_WEIGHT (传输给收件人，特定于仅自动完成流)   <br/> |0x60040003  <br/> |此自动完成条目的权重。 权重用于确定在匹配自动完成列表时自动完成条目发生的顺序。 权重较高的条目显示在权重较低的条目之前。 完整的自动完成列表按此属性排序。 权重会随着时间的推移而定期减小，并且当用户向此收件人发送电子邮件时会增加。 有关此属性的详细信息，请参阅本主题稍后的说明。  <br/> |
   
PR_NICK_NAME_WEIGHT
  
自动完成流中的行集按 PR_NICK_NAME_WEIGHT 属性的降序排序，自动完成流应始终保留此排序特征。 因此，对行的权重的任何更改还应确保行的位置保持整组行的排序顺序。 行集的任何添加内容都应插入到正确的位置以保持排序顺序。
  
此权重的最小值为 0x1，最大值为 LONG_MAX。 权重的其他任何值都被视为无效。
  
当 Outlook 2007 向收件人发送邮件或解析收件人时，该收件人的权重将增加 0x2000。
