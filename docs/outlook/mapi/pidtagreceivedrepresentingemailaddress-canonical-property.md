...
这些属性是接收用户所代表的邮件用户的地址属性示例。 它们必须由传入传输提供程序进行设置，该提供商还负责授权或验证代理。 如果未表示任何消息用户，则这些属性应设置为 **PR_RECEIVED_BY_EMAIL_ADDRESS** ([PidTagReceivedByEmailAddress](pidtagreceivedbyemailaddress-canonical-property.md)) 中的电子邮件地址。
  
答复代表其他客户端接收的邮件的客户端应用程序应该将这些属性从收到的邮件复制到 **回复** 的 PR_SENT_REPRESENTING_EMAIL_ADDRESS ([PidTagSentRepresentingEmailAddress](pidtagsentrepresentingemailaddress-canonical-property.md)) 属性中。
