...
此属性是邮件的原始发件人的地址属性之一。 它在会话线程中使用。
  
代表另一个客户端发送邮件的客户端应用程序应在第一次提交邮件时将此属性设置为**PR_SENT_REPRESENTING_SEARCH_KEY** ([PidTagSentRepresentingSearchKey](pidtagsentrepresentingsearchkey-canonical-property.md)) 属性的值。 设置后, 决不能更改它。
