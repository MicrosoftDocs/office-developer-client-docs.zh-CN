...
此属性由邮件存储区为所有文件夹和邮件计算。
  
对于邮件存储根文件夹, 此属性包含该文件夹自己的条目标识符。
  
 **PR_PARENT_DISPLAY**([PidTagParentDisplay](pidtagparentdisplay-canonical-property.md)), 并且此属性不相互关联。 它们属于完全不同的上下文。
