...
返回 [一个 MAPIERROR](mapierror.md) 结构，其中包含有关邮件站点对象发生的上一个错误的信息。 
...
**IMAPIMessageSite：：GetLastError** 方法提供有关失败之前的方法调用的信息。 调用方可以通过在对话框中包含 **MAPIERROR** 结构的数据来为用户提供有关错误的详细信息。 
