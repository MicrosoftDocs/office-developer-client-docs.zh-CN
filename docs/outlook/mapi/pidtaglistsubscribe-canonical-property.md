...
包含多用途 Internet 邮件扩展 (MIME) 邮件的List-Subscribe字段。
...
若要生成List-Subscribe字段，客户端必须将这些属性的值设置为所需的值。 MIME 编写器必须将这些属性的值复制到List-Subscribe字段中。
  
若要设置与服务器相关的类似属性的值，MIME 客户端必须编写下表中指定的头字段。
