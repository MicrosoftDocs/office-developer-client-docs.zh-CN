...
包含邮件第一个版本的发件人的电子邮件地址，即转发或答复前的邮件。
...
这些属性是邮件的原始发件人的地址属性示例。 首次提交邮件时，客户端应用程序应该将此属性设置为 PR_SENDER_EMAIL_ADDRESS ([PidTagSenderEmailAddress](pidtagsenderemailaddress-canonical-property.md)) 。  转发或答复邮件时从不更改。
