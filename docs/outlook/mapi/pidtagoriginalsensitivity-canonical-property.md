...
包含由第一个版本的邮件（即转发或答复前的邮件）的发件人分配的敏感度值。
...
客户端应用程序应在首次提交邮件时将此属性设置为与 PR_SENSITIVITY  ([PidTagSensitivity](pidtagsensitivity-canonical-property.md)) 属性相同的值。 以后不应更改它。
  
传输提供程序使用此属性来保护复制条目的敏感度。 例如，它使用户可以阻止修改原始邮件文本，以转发或回复最初标记为 **"SENSITIVITY_PRIVATE"。**
