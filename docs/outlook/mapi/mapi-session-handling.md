...
必须先建立会话，然后才能与服务提供商和基础邮件系统通信。 MAPI 会话是一个从客户端到其他 MAPI 组件的链接。 成功启动会话后，MAPI 会向客户端返回指向会话对象的指针，即实现 **IMAPISession 接口** 的对象。 有关详细信息，请参阅 [IMAPISession ： IUnknown](imapisessioniunknown.md)。 可以使用 **IMAPISession** 接口的方法访问通讯簿和邮件存储提供程序的对象、访问多个表、显示表单、设置传输提供程序属性以及执行配置文件和邮件服务管理。 
  
## <a name="in-this-section"></a>本节内容

[启动 MAPI 会话](starting-a-mapi-session.md)
  
> 介绍如何启动 MAPI 会话，并包括指向包含更多详细信息的主题的链接。
