...
*Error* 参数是对 ADO [Error](error-object-ado.md) 对象的引用。当 *Status* 参数设置为 **adStatusErrorsOccurred** 时，**Error** 对象将包含有关操作失败原因的详细信息。如果与 Complete 事件关联的 Will 事件已经通过将 *Status* 参数设置为 **adStatusCancel** 而取消了操作，则 Error 对象将始终设置为 **adErrOperationCancelled**。
...
相反，只需将 *adStatus* 设置为 **adStatusUnwantedEvent** 一次，即可请求没有 **adReason** 参数的事件处理程序停止接收事件通知。
