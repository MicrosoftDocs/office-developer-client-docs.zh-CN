...
在设置 [CommandText](prepared-property-ado.md) 属性时，如果 **Command** 对象的 **Prepared** 属性设置为 **True** ，且 **Command** 对象绑定到打开的连接，则在调用 [Execute](https://msdn.microsoft.com/library/jj248785\(v=office.15\)) 或 **Open** 方法时，ADO 会准备查询（即提供程序所存储的查询的已编译格式）。
