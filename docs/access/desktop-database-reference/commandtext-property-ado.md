---
<<<<<<< 标头标题： CommandText 属性 (ADO) TOCTitle: CommandText 属性 (ADO) === 标题： CommandText 属性 (ADO) TOCTitle: CommandText 属性 (ADO)
>>>>>>> 母版页 ms:assetid: 0debec1c-068f-0aea-fce8-e61aa39c5907 ms:mtpsurl: https://msdn.microsoft.com/library/JJ248859(v=office.15) ms:contentKeyID: 48543234 ms.date: 09/18/2015 mtps_version: office.15.aspx f1_keywords:
...
<<<<<<< 标头如果将**Command**对象的[已准备](prepared-property-ado.md)属性设置为**True**和**Command**对象绑定到打开的连接设置**CommandText**属性时，ADO 准备查询 （即编译窗体的查询的提供程序存储） 时调用的[Execute](https://msdn.microsoft.com/library/jj248785\(v=office.15\))或**Open**方法。
=== ADO 如果**Command**对象的[已准备](prepared-property-ado.md)属性设置为**True** ，设置**CommandText**属性时，将**Command**对象绑定到打开的连接，准备查询 (也就是说，编译窗体的存储提供程序的查询） 时调用的[Execute](https://docs.microsoft.com/office/vba/access/concepts/miscellaneous/execute-method-ado-command)或**Open**方法。
>>>>>>> master
