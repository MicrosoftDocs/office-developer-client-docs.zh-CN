...
---
# <a name="databasecreateproperty-method-dao"></a>CreateProperty 方法 (DAO)
...
<td><p>可选</p></td>
<td><p><strong>Variant</strong></p></td>
<td><p>一个 <strong>Variant</strong>（<strong>Boolean</strong> 子类型），用于指示 <strong>Property</strong> 是否为 DDL 对象。 默认值为 False。 如果 DDL 为 True, 则用户不能更改或删除此<strong>属性</strong>对象, 除非他们具有 dbSecWriteDef 权限。</p></td>
</tr>
</tbody>
</table>
...
> [!NOTE]
> 如果省略了 DDL 参数, 它的默认值为 False (非 DDL)。 由于没有公开相应的 DDL 属性，必须删除要从 DDL 更改为非 DDL 的 **Property** 对象，然后重新创建该对象。
