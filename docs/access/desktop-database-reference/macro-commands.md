---
title: 宏命令 （访问桌面数据库参考 （英文）
TOCTitle: Macro commands
ms:assetid: 24e7f34d-7902-4644-bb67-33689d9895c0
ms:mtpsurl: https://msdn.microsoft.com/library/Dn124057(v=office.15)
ms:contentKeyID: 52071694
ms.date: 09/18/2015
mtps_version: v=office.15
localization_priority: Priority
ms.openlocfilehash: e0454b876b8d81d929252b1561ecb881afe230c6
ms.sourcegitcommit: d6695c94415fa47952ee7961a69660abc0904434
ms.translationtype: Auto
ms.contentlocale: zh-CN
ms.lasthandoff: 01/17/2019
ms.locfileid: "28715058"
---
# <a name="macro-commands"></a>宏命令

**适用于**： Access 2013、 Office 2013

- [CancelEvent 宏操作](cancelevent-macro-action.md)
- [Echo 宏操作](echo-macro-action.md)
- [OpenVisualBasicModule 宏操作](openvisualbasicmodule-macro-action.md)
- [RemoveAllTempVars 宏操作](removealltempvars-macro-action.md)
- [RemoveTempVar 宏操作](removetempvar-macro-action.md)
- [RunCode 宏操作](runcode-macro-action.md)
- [RunDataMacro 宏操作](rundatamacro-macro-action.md)
- [RunMacro 宏操作](runmacro-macro-action.md)
- [RunMenuCommand 宏操作](runmenucommand-macro-action.md)
- [SetTempVar 宏操作](settempvar-macro-action.md)
- [SingleStep 宏操作](singlestep-macro-action.md)
- [StartNewWorkflow 宏操作](startnewworkflow-macro-action.md)
- [StopAllMacros 宏操作](stopallmacros-macro-action.md)
- [StopMacro 宏操作](stopmacro-macro-action.md)
- [WorkflowTasks 宏操作](workflowtasks-macro-action.md)

