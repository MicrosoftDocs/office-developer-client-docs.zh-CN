...
---
# <a name="field2validationrule-property-dao"></a>Field2 属性 (DAO)
...
> [!NOTE]
> 如果将该属性设置为与非整数值串联在一起的字符串, 并且系统参数指定的非美国十进制字符 (如逗号) (例如, strRule = "PRICE &gt; " &amp; lngPrice 和 lngPrice = 125, 50), 则会导致错误您的代码尝试验证任何数据。 这是因为在连接过程中，将使用系统的默认小数字符，将数字转换为字符串，并且 Microsoft Access 数据库引擎 SQL 只接受美国格式小数字符。
