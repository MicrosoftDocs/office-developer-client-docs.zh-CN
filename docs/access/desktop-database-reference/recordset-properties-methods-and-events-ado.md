---
title: Recordset 属性、 方法和事件 (ADO)
TOCTitle: Properties, Methods, and Events
ms:assetid: aca81c6c-5b08-13e9-5bf5-3561a173f5bc
ms:mtpsurl: https://msdn.microsoft.com/library/JJ249806(v=office.15)
ms:contentKeyID: 48547019
ms.date: 09/18/2015
mtps_version: v=office.15
ms.openlocfilehash: ca86e3b852486a92b8554c56ebe5f21c23363bca
ms.sourcegitcommit: d7248f803002b31cf7fc561b03530199a9b0a8fd
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 11/02/2018
ms.locfileid: "25929347"
---
# <a name="recordset-properties-methods-and-events-ado"></a>Recordset 属性、 方法和事件 (ADO)


**适用于**： Access 2013、 Office 2013

## <a name="propertiescollections"></a>属性/集合

- [AbsolutePage 属性](absolutepage-property-ado.md)

- [AbsolutePosition 属性](absoluteposition-property-ado.md)

- [ActiveCommand 属性](activecommand-property-ado.md)

- [ActiveConnection 属性](activeconnection-property-ado.md)

- [BOF、EOF 属性](bof-eof-properties-ado.md)

- [Bookmark 属性](bookmark-property-ado.md)

- [CacheSize 属性](cachesize-property-ado.md)

- [CursorLocation 属性](cursorlocation-property-ado.md)

- [CursorType 属性](cursortype-property-ado.md)

- [DataMember 属性](datamember-property-ado.md)

- [DataSource 属性](datasource-property-ado.md)

- [EditMode 属性](editmode-property-ado.md)

- [Fields 集合](fields-collection-ado.md)

- [Filter 属性](filter-property-ado.md)

- [Index 属性](index-property-ado.md)

- [LockType 属性](locktype-property-ado.md)

- [MarshalOptions 属性](marshaloptions-property-ado.md)

- [MaxRecords 属性](maxrecords-property-ado.md)

- [PageCount 属性](pagecount-property-ado.md)

- [PageSize 属性](pagesize-property-ado.md)

- [Properties 集合](properties-collection-ado.md)

- [RecordCount 属性](recordcount-property-ado.md)

- [Sort 属性](sort-property-ado.md)

- [Source 属性 (ADO Recordset)](source-property-ado-recordset.md)

- [State 属性](state-property-ado.md)

- [Status 属性 (ADO Recordset)](status-property-ado-recordset.md)

- [StayInSync 属性](stayinsync-property-ado.md)

## <a name="methods"></a>方法

- [AddNew 方法](addnew-method-ado.md)

- [Cancel 方法](cancel-method-ado.md)

- [CancelBatch 方法](cancelbatch-method-ado.md)

- [CancelUpdate 方法](cancelupdate-method-ado.md)

- [Clone 方法](clone-method-ado.md)

- [Close 方法](close-method-ado.md)

- [CompareBookmarks 方法](comparebookmarks-method-ado.md)

- [Delete 方法 (ADO Recordset)](delete-method-ado-recordset.md)

- [Find 方法](find-method-ado.md)

- [GetRows 方法](getrows-method-ado.md)

- [GetString 方法](getstring-method-ado.md)

- [Move 方法](move-method-ado.md)

- [MoveFirst、MoveLast、MoveNext 和 MovePrevious 方法](movefirst-movelast-movenext-and-moveprevious-methods-ado.md)

- [NextRecordset 方法](nextrecordset-method-ado.md)

- [Open 方法 (ADO Recordset)](open-method-ado-recordset.md)

- [Requery 方法](requery-method-ado.md)

- [Resync 方法](resync-method-ado.md)

- [Save 方法](save-method-ado.md)

- [Seek 方法](seek-method-ado.md)

- [Supports 方法](supports-method-ado.md)

- [Update 方法](update-method-ado.md)

- [UpdateBatch 方法](updatebatch-method-ado.md)

## <a name="events"></a>事件

- [EndOfRecordset 事件](endofrecordset-event-ado.md)

- [FetchComplete 事件](fetchcomplete-event-ado.md)

- [FetchProgress 事件](fetchprogress-event-ado.md)

- [WillChangeField 和 FieldChangeComplete 事件](willchangefield-and-fieldchangecomplete-events-ado.md)

- [WillChangeRecord 和 RecordChangeComplete 事件](willchangerecord-and-recordchangecomplete-events-ado.md)

- [WillChangeRecordset 和 RecordsetChangeComplete 事件](willchangerecordset-and-recordsetchangecomplete-events-ado.md)

- [WillMove 和 MoveComplete 事件](willmove-and-movecomplete-events-ado.md)

