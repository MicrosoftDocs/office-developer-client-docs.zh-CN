...
<<<<<<< 标头**RDSServer.DataFactory**对象专为接收客户端请求的服务器端自动化对象。 在 Internet 实现中，它驻留在 Web 服务器上，并由 ADISAPI 组件实例化。 **RDSServer.DataFactory** 对象提供对指定数据源的读写访问权限，但不包含任何验证或业务规则逻辑。
...
<a name="if-you-want-your-web-application-to-handle-task-specific-server-side-processing-you-can-replace-the-rdsserverdatafactory-with-a-custom-business-object"></a>如果希望 Web 应用程序管理特定于任务的服务器端处理，可以使用自定义业务对象替代 **RDSServer.DataFactory** 。
=======
