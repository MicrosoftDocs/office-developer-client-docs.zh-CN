...
> [!TIP]
> <P>您可以使用的<STRONG>重复次数</STRONG>和<STRONG>重复表达式</STRONG>参数确定宏将运行的次数：</P>
...
> <UL>
> <LI>
> <P>可以调用同一个宏组或者另一个宏组中的宏。</P>
> <LI>
> <P>可以嵌套宏。也就是说，您可以运行宏 A，宏 A 继而调用宏 B，依此类推。在每种情况下，当被调用的宏运行完后，Access 将继续运行调用它的宏并运行该宏中的下一个操作。</P></LI></UL>
