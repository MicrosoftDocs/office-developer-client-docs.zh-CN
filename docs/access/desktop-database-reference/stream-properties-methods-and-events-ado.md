---
title: 流属性、 方法和事件 (ADO)
TOCTitle: Properties, Methods, and Events
ms:assetid: 12542605-9520-f0a9-09e4-9eac6b37c35e
ms:mtpsurl: https://msdn.microsoft.com/library/JJ248897(v=office.15)
ms:contentKeyID: 48543339
ms.date: 09/18/2015
mtps_version: v=office.15
ms.openlocfilehash: d710944375a7fbac22371d1b18f69424b4bcf061
ms.sourcegitcommit: d7248f803002b31cf7fc561b03530199a9b0a8fd
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 11/02/2018
ms.locfileid: "25931034"
---
# <a name="stream-properties-methods-and-events-ado"></a>流属性、 方法和事件 (ADO)


**适用于**： Access 2013、 Office 2013

## <a name="properties"></a>属性

- [Charset 属性](charset-property-ado.md)

- [EOS 属性](eos-property-ado.md)

- [LineSeparator 属性](lineseparator-property-ado.md)

- [Mode 属性](mode-property-ado.md)

- [Position 属性](position-property-ado.md)

- [Size 属性 (ADO Stream)](https://msdn.microsoft.com/library/jj250128\(v=office.15\))

- [State 属性](state-property-ado.md)

- [Type 属性 (ADO Stream)](type-property-ado-stream.md)

## <a name="methods"></a>方法

- [Cancel 方法](cancel-method-ado.md)

- [Close 方法](close-method-ado.md)

- [CopyTo 方法](copyto-method-ado.md)

- [Flush 方法](flush-method-ado.md)

- [LoadFromFile 方法](loadfromfile-method-ado.md)

- [Open 方法 (ADO Stream)](open-method-ado-stream.md)

- [Read 方法](read-method-ado.md)

- [ReadText 方法](readtext-method-ado.md)

- [SaveToFile 方法](savetofile-method-ado.md)

- [SetEOS 方法](seteos-method-ado.md)

- [SkipLine 方法](skipline-method-ado.md)

- [Stat 方法](stat-method-ado.md)

- [Write 方法](write-method-ado.md)

- [WriteText 方法](writetext-method-ado.md)

## <a name="events"></a>事件

无。

