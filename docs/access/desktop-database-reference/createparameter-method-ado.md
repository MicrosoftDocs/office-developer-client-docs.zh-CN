...
**Set***参数* = *命令*。CreateParameter (*名称*、*类型*、*方向*、*大小*、*值*)
...
如果在 *Type* 参数中指定长度可变的数据类型，则在将 **Parameter** 对象追加到 **Parameters** 集合中之前，必须传递 *Size* 参数或设置该对象的 [Size](size-property-ado.md) 属性；否则，将发生错误。
