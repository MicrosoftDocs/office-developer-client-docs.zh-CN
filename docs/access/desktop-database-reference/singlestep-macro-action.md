...
> [!NOTE]
> <P>[!注释] 如果一直进行单步执行直到宏结束而不单击 <STRONG>"继续"</STRONG>，单步执行在宏执行完时仍将有效。后续运行的任何宏都将从单步执行模式下开始。要关闭单步执行，请单击 <STRONG>"单步执行宏"</STRONG>对话框中的 <STRONG>"继续"</STRONG>，或者在设计视图中打开一个宏，然后在 <STRONG>"设计"</STRONG>选项卡上的 <STRONG>"工具"</STRONG>组中单击 <STRONG>"单步执行"</STRONG>使其不再被选中。</P>
