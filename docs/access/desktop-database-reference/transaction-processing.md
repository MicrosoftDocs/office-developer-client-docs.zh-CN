...
使用 **IsolationLevel** 属性可以设置 **Connection** 对象上事务的隔离级别。 直到下次调用 [BeginTrans](begintrans-committrans-and-rollbacktrans-methods-ado.md) 方法时，该设置才会生效。 如果请求的隔离级别不可用，则提供程序将返回下一个更高的隔离级别。 有关有效值的更多详细信息, 请参阅 ADO 程序员参考中的**IsolationLevel**属性。
