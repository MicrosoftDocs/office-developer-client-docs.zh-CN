...
使用 **Table** 对象的属性和集合，可以执行下列操作：
  ...
  - 使用 [Type](https://msdn.microsoft.com/library/jj250042\(v=office.15\)) 属性确定表的类型。
...
> [!NOTE]
> <P>[!注释] 您的数据提供程序可能并不支持 <STRONG>Table</STRONG> 对象的所有属性。如果您设置了提供程序不支持的属性的值，将发生错误。对于新的 <STRONG>Table</STRONG> 对象，将其追加到集合时，将发生错误。对于现有对象，在设置属性时，将发生错误。</P>
