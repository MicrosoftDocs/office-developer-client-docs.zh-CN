---
title: Hierarchies 集合 （ADO MD 参考 （英文）
TOCTitle: Hierarchies collection (ADO MD)
ms:assetid: d073a1ca-0f5a-48e0-84da-6a3c62c8cda9
ms:mtpsurl: https://msdn.microsoft.com/library/JJ250040(v=office.15)
ms:contentKeyID: 48547839
ms.date: 09/18/2015
mtps_version: v=office.15
ms.openlocfilehash: 6f02d4d64b0c99947aa0594e86ef3b251bf6502d
ms.sourcegitcommit: d7248f803002b31cf7fc561b03530199a9b0a8fd
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 11/02/2018
ms.locfileid: "25927954"
---
# <a name="hierarchies-collection-ado-md-reference"></a>Hierarchies 集合 （ADO MD 参考 （英文）

**适用于**： Access 2013、 Office 2013

- [Hierarchies 集合 (ADO MD)](hierarchies-collection-ado-md.md)
- [层次结构集合的属性、 方法和事件 (ADO MD)](hierarchies-collection-properties-methods-and-events-ado-md.md)

