...
您可以使用**RequeryRecords**操作通过重新查询视图的源来刷新、排序和筛选活动视图中的数据。 
...
若要按降序顺序对记录进行排序, 请在*OrderBy*参数表达式的末尾输入 DESC。 例如, 若要按联系人姓名的降序对客户记录进行排序, 请将*OrderBy*参数设置为 "联系人姓名 DESC"。 
  
若要按姓氏降序以及名字升序对名称进行排序, 请将*OrderBy*参数设置为 "LastName DESC, FirstName ASC"。 
