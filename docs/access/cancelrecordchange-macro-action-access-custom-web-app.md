...
---
# <a name="cancelrecordchange-macro-action-access-custom-web-app"></a><span data-ttu-id="6aab8-103">CancelRecordChange 宏操作 （访问自定义 web 应用程序）</span><span class="sxs-lookup"><span data-stu-id="6aab8-103">CancelRecordChange Macro Action (Access custom web app)</span></span>
...
<span data-ttu-id="6aab8-109">在调用 **CancelRecordChange** 操作时， **CreateRecord** 或 **EditRecord** 数据块将立即退出。</span><span class="sxs-lookup"><span data-stu-id="6aab8-109">When you call the **CancelRecordChange** action, the **CreateRecord** or **EditRecord** data block is exited immediately.</span></span> 
